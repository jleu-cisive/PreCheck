...
/*
	...
	Description:	Removed CTE and replaced it with a subquery.  A Index on OrderNumber Might be useful
	Modified by Arshan Gouri on 07/20/2023 for HDT #101824 HCA: DT PDF Button Not Appearing on Tokenized Results Page
*/


CREATE VIEW [Enterprise].[vwReportStatus]
...
SELECT
	...
	DrugScreenStatus=
						CASE 
							WHEN (ISNULL(O.HasDrugScreen, 0) = 0 AND ISNULL(ci.OCHS_CandidateInfoID,D.TID) IS NULL) THEN NULL 
	...
	DrugScreenResult=
						CASE 
							WHEN (ISNULL(O.HasDrugScreen, 0) = 0 AND ISNULL(D.TID,d1.tid) IS NULL) THEN NULL 
	...
	ImmunizationStatus= 
						CASE 
							WHEN ISNULL(O.HasImmunization,0)=1 THEN 'F' 
							ELSE NULL 
						END,
	OrderStatus=
				CASE 
	...
	--dateofbirth=ISNULL(CAST(a.dob AS DATE),ci.dob),
	dateofbirth=CAST(a.dob AS DATE),
...
FROM 
	(
		SELECT 
			...
			, ApDate
			, ApStatus
			, CreatedDate=ISNULL(CreatedDate, '1/1/1900')
			, CLNO
			, DOB
			, SSN
			, I94
...
LEFT OUTER JOIN dbo.OCHS_CandidateInfo CI
	ON A.APNO=CI.APNO --AND a.clno=ci.clno Commented by Arshan for HDT 101824
