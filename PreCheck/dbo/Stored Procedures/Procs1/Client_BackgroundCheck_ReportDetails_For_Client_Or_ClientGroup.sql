...
CREATE PROCEDURE [dbo].[Client_BackgroundCheck_ReportDetails_For_Client_Or_ClientGroup] 
...
BEGIN
	...
	SELECT 
		...
		,RA.Affiliate
		,A.UserID AS [CAM]
		,A.First AS [Applicant FirstName]
		,A.Last AS [Applicant LastName]
		,A.Middle AS [Applicant MiddleName]
...
END
	...
	SELECT DISTINCT [Recruiter]
		...
		,IsHidden
		,IsOnReport
		,ISNULL(QRY.[Private Notes],#AppData.[Private Notes])
		,ISNULL(QRY.[Public Notes],#AppData.[Public Notes])
		,[TAT]
		,IsOneHR
	FROM (
