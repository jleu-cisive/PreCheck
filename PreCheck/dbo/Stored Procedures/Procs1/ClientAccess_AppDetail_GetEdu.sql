...
-- Description:	 Gets Education Verification Details for the client in Check Reports
-- EXEC [dbo].[ClientAccess_AppDetail_GetEdu] 5723818, 1, 12771
...
CREATE PROCEDURE [dbo].[ClientAccess_AppDetail_GetEdu]
@apno int,
...
Select educatid,school,	
		CASE WHEN sectstat.code in('0', '9', 'A','R') and LOWER(@HasETA) = 'true' THEN 
				...
				END 
		ELSE isnull((case when Isnull(@AdjudicationProcess,0) =1 then  ( Case WHEN A.Apstatus ='F' AND Educat.sectstat in ('4','5') THEN 'CLEAR' ELSE (
...
FROM educat 
INNER JOIN APPL A on a.apno  = educat.apno
LEFT OUTER JOIN SECTSTAT ON educat.sectstat = sectstat.code 
left join dbo.SectSubStatus sss (nolock) on educat.SectStat= sss.SectStatusCode and sss.ApplSectionID=2 and educat.SectSubStatusID = sss.SectSubStatusID 
LEFT JOIN dbo.ClientAdjudicationStatus AdjStatus ON educat.ClientAdjudicationStatus = AdjStatus.ClientAdjudicationStatusID --and AdjStatus.CLNO = @clno
LEFT JOIN dbo.ClientAdjudicationStatusCustom AdjStatusCustom ON AdjStatusCustom.clno = a.clno and educat.ClientAdjudicationStatus = AdjStatusCustom.ClientAdjudicationStatusID
LEFT JOIN dbo.ApplSectionsETA eta ON eta.Apno = A.APNO AND eta.SectionKeyID = Educat.EducatID AND eta.ApplSectionID = 2
where isonreport = 1 
and ishidden = 0 
and educat.apno =  @apno
