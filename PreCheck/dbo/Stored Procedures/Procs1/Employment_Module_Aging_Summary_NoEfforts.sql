...
/*
Procedure Name : Employment_Module_Aging_Summary_NoEfforts
Requested By: Dana Sangerhausen
Developer: Deepak Vodethela
Execution : EXEC Employment_Module_Aging_Summary_NoEfforts
*/

CREATE PROCEDURE [dbo].[Employment_Module_Aging_Summary_NoEfforts]
AS

IF OBJECT_ID('tempdb..#WeekDayRecords') IS NOT NULL
BEGIN
    DROP TABLE #WeekDayRecords
END

IF OBJECT_ID('tempdb..#WeekEndRecords') IS NOT NULL
BEGIN
    DROP TABLE #WeekEndRecords
END

IF OBJECT_ID('tempdb..#InvestigatorTotals') IS NOT NULL
BEGIN
    DROP TABLE #InvestigatorTotals
END

IF OBJECT_ID('tempdb..#InvestigatorAggrigatesForNulls') IS NOT NULL
BEGIN
    DROP TABLE #InvestigatorAggrigatesForNulls
END

IF OBJECT_ID('tempdb..#AgingRecordsByInvestigator') IS NOT NULL
BEGIN
    DROP TABLE #AgingRecordsByInvestigator
END

-- Get Weekday Records
SELECT	E.Investigator,
		CreatedDate,
		DATENAME(dw, CreatedDate) AS NameOfTheDay,
		dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE()) AS NoOfHours,
		CASE --WHEN (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '23.99' THEN 0
			 WHEN ((dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) > '0.00' AND (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '47.99') THEN 1
			 WHEN ((dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) > '47.99' AND (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '71.99') THEN 2
			 WHEN ((dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) > '71.99' AND (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '95.99') THEN 3
			 WHEN ((dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) > '95.99' AND (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '119.99') THEN 4
			 WHEN ((dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) > '119.99' AND (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '143.99') THEN 5
			 WHEN ((dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) > '143.99' AND (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '167.99') THEN 6
			 WHEN ((dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) > '167.99' AND (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '191.99') THEN 7
		ELSE 8
		END DaysPassed,
...
INNER JOIN dbo.Users AS u(NOLOCK) ON E.Investigator = U.UserID AND U.empl = 1
WHERE web_status = '0'
...
GROUP BY E.Investigator,
		 CreatedDate,
		 dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE()),
		 CASE --WHEN (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '23.99' THEN 0
			 WHEN ((dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) > '0.00' AND (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '47.99') THEN 1
			 WHEN ((dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) > '47.99' AND (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '71.99') THEN 2
			 WHEN ((dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) > '71.99' AND (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '95.99') THEN 3
...
WHERE web_status = '0'
  ...
  AND IsOnReport = 1
  AND CreatedDate >= DATEADD(WEEK, -2, GETDATE()) 
  AND DATENAME(dw, CreatedDate) IN ('Saturday', 'Sunday')
GROUP BY E.Investigator,
		 CreatedDate,
		 DATENAME(dw, createddate),
		 dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE()),
		CASE --WHEN (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '23.99' THEN 0
			 WHEN ((dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) > '0.00' AND (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '47.99') THEN 1
			 WHEN ((dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) > '47.99' AND (dbo.GetHoursExcludingWeekEnd(CreatedDate, GETDATE())) <= '71.99') THEN 2
