...
-- Description: Ticketno - 101869 - Update Logic on QReport
    EXEC [dbo].[Count_of_Reports_Qualifying_for_TBF_by_Date_rollback_cpy1] '07/01/2023','08/04/2023'
*/
-- =============================================    

CREATE PROCEDURE [dbo].[Count_of_Reports_Qualifying_for_TBF_by_Date_rollback_cpy1] 
(
@StartDate DATETIME,
@EndDate DATETIME
)
    ...
    WHERE RowNumber = 1
	  ...
	  /*
		...
		FROM dbo.ChangeLog AS C WITH (NOLOCK)
			...
			INNER JOIN #tmp t WITH (NOLOCK)
				ON X.APNO = t.APNO
		  and C.ChangeDate < DATEADD(SECOND,3499,T.[Date Report Closed])  --code added for ticket no -101869 
