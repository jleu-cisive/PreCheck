-- Alter Procedure Billing_PassThroughCharges
...
CREATE PROCEDURE [dbo].[Billing_PassThroughCharges]
(
	@StartDate datetime
	, @EndDate datetime
	, @CLNO smallint
...
INNER JOIN (
				...
				(
					...
					AND AA.IsActive = 1 and A.Billed = 0 AND A.CLNO = @CLNO
           ...
           WHERE A.Billed = 0 AND C2.IsHidden = 0 AND A.CLNO = @CLNO
...
FROM   dbo.TblCounties C
              ...
              INNER JOIN dbo.Client Cl ON Cl.CLNO = A.CLNO  AND A.Billed = 0 AND Cl.CLNO = @CLNO
...
--add surcharges
EXEC Billing_PackageSurcharge 2,null,@CLNO
