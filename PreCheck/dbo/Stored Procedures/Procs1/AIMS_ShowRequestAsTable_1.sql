 --AIMS_ShowRequestAsTable_1 329148
CREATE procedure [dbo].[AIMS_ShowRequestAsTable_1]
(@logid int)
as
declare @xml xml
...
IF(@xml IS NULL)
	BEGIN
		set @xml = (select Request from [Precheck_Archive].dbo.DataXtract_Logging (nolock) where DataXtract_LoggingId = IsNull(@logId,0))
	END
...
IF(@xml IS NULL)
	BEGIN
		set @xml = (select Request from [Precheck_Archive].dbo.DataXtract_Logging_2016 (nolock) where DataXtract_LoggingId = IsNull(@logId,0))
	END
