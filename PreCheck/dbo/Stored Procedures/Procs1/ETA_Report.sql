...
-- Description:	HDT #70144 ETA Report for Dignity and CHI - Logic is based of the stored procedure - Precheck.[dbo].ETA_Report_for_HCA
-- Modification : Done by Humera Ahmed on 9/21/2021 for HDT# 18440. To include all pending reports and not 30 days worth of data.
-- Modification : Done by Humera Ahmed on 10/12/2021. Scheduled report was failing because 2 APNO had NULL Certification Date (Report Start Date). 
--                Made the non nullable Report Start Date column to nullable in table variable @tmpETAForHCA
-- =============================================
CREATE PROCEDURE [dbo].[ETA_Report]
...
BEGIN
	...
	INSERT INTO @tmpETAForHCA
		SELECT DISTINCT
			   ...
			   A.ClientApplicantNO AS [Taleo Candidate ID], 
			   ...
			   CASE when DATEDIFF(d,@ETAPASTTRIGGER,ETADate) < 0 then 'ETA Unavailable' else cast([dbo].[ElapsedBusinessDays_2](CONVERT(varchar,@EndDate, 101),CONVERT(varchar, ETADate, 101)) as varchar(10)) end AS [ETADays],
		...
		SELECT  t.[Client Number], t.[Client Name], t.[Report Number], t.[Applicant Name],t.[Taleo Candidate ID], t.[Package Ordered], t.[Process Level], 
