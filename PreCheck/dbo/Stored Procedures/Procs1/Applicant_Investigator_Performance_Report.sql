...
-- exec [Applicant_Investigator_Performance_Report] '09/13/2022', '09/13/2022'  
  
CREATE Procedure [dbo].[Applicant_Investigator_Performance_Report]  
(  
...
WHERE     (CreatedDate> CONVERT(DATETIME, @StartDate, 102)) and CreatedDate< = dateadd(s,-1,dateadd(d,1,@EndDate))  
		  ...
		  and QueueType not in ('P','All') --added by Mainak for ticket no.13307
...
SELECT   Newvalue  into #tempChangeLog3     --Commented by Mainak for ticket no.13307   --code added by vairavan for ticket no - 13307
--SELECT   UserID Newvalue into #tempChangeLog3 --Added by Mainak for ticket no.13307 --code commented by vairavan for ticket no - 13307 
FROM         dbo.ChangeLog (Nolock)  
WHERE     
(ChangeDate > CONVERT(DATETIME, @StartDate, 102)) and ChangeDate< = dateadd(s,-1,dateadd(d,1,@EndDate))  
  
and TableName = 'Appl.Investigator' and OldValue = ''   
--order by ID,UserID  
