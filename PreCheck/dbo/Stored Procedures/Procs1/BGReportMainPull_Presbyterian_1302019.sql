...
-- Execution: EXEC [BGReportMainPull_Presbyterian] 7898, '01/1/2010','1/1/2019',1
-- =============================================

create PROCEDURE [dbo].[BGReportMainPull_Presbyterian_1302019]
...
--SET @CLNO = 7898;
SET @StartDate = '1/01/2008'--DATEADD(m,-4,getdate());
Set @EndDate = '01/03/2019';
...
If(@UseHevnDb = 1)
 BEGIN
	...
	SELECT br.backgroundreportid,a.apno,a.CLNO as clientid,r.*
	into #tempResend
	 --update R set Resend = 1
	from reportuploadlog r WITH (NOLOCK) 
	inner join reportuploadvolume v WITH (NOLOCK)  on r.ReportUploadVolumeID = v.ReportUploadVolumeID and v.ForClient = 7898
	inner join BackgroundReports.dbo.BackgroundReport br WITH (NOLOCK) on r.ReportID = br.backgroundreportid
	inner join Appl a WITH (NOLOCK) on br.apno = a.apno
	where v.CreatedDate > '1/1/2018'
	-- r.ReportID = br.backgroundreportid and r.resend = 0 
	 AND r.ReportType = 1
	 and a.CLNO not in ( select clno from clienthierarchybyservice WITH (NOLOCK) where parentclno = 7898 and refhierarchyserviceid = 1)
		...
		FROM (
			...
			WHERE (F.ParentEmployerID=@CLNO OR F.EmployerID=@CLNO)
					...
					AND (a.clno = @CLNO OR a.clno in (select clno from clienthierarchybyservice WITH (NOLOCK) where parentclno = @CLNO and refhierarchyserviceid = 1 ))
					 and a.APNO in ( Select ReportID from #tempResend)
			) as idTable 
			...
			--AND (SELECT Count(*) from reportuploadlog r WITH (NOLOCK) where r.ReportID = br.backgroundreportid and r.resend = 0 AND r.ReportType = 1) = 0
						 and idTable.APNO in ( Select ReportID from #tempResend)
	...
	SELECT DISTINCT  a.apno,cast(a.clno as int) as clno ,'000000000' employeeNumber, 'NonGrouped' as ClientFacilityGroup
			...
			--AND (SELECT Count(*) from reportuploadlog r WITH (NOLOCK) where r.ReportID = br.backgroundreportid and r.resend = 0 AND r.ReportType = 1) = 0
			 and idTable.APNO in ( Select ReportID from #tempResend)
	--SELECT * FROM #TempNonemployees
	SELECT distinct FolderID, clno, ReportID, employeeNumber, ClientFacilityGroup		
	FROM(
		SELECT  FolderID, clno,  ReportID,employeeNumber,  ClientFacilityGroup FROM #tmp
	UNION ALL
		SELECT  FolderID, clno,  ReportID,employeeNumber,  ClientFacilityGroup FROM #TempNonemployees
	UNION ALL
		select  idtable.apno as FolderID,idtable.clno, br.backgroundreportid AS ReportID,idtable.employeeNumber, isnull(ClientFacilityGroup,'NonGrouped') as ClientFacilityGroup --schapyala returning emptystring for facilitygroup as this is not used for BG purposes - 05/04/2015
		from (
			SELECT DISTINCT  a.apno,cast(a.clno as int) as clno ,'000000000' employeeNumber, 'NonGrouped' as ClientFacilityGroup
			...
			WHERE 
					...
					AND (a.clno = @CLNO OR a.clno in (select clno from clienthierarchybyservice WITH (NOLOCK) where parentclno = @CLNO and refhierarchyserviceid = 1 ))
					AND A.APNO NOT IN (SELECT FolderID FROM #tmp)
					AND A.APNO NOT IN (SELECT FolderID FROM #TempNonemployees)
			) as idTable 
			INNER JOIN BackgroundReports.dbo.BackgroundReport br WITH (NOLOCK) on br.apno = idTable.apno
			and br.CreateDate = (Select MAX(createdate) from BackgroundReports.dbo.BackgroundReport WITH (NOLOCK) where apno = idTable.apno)
			AND (SELECT Count(*) from reportuploadlog r WITH (NOLOCK) where r.ReportID = br.backgroundreportid and r.resend = 0 AND r.ReportType = 1) = 0
 ...
 End
   ...
   DROP TABLE #tmp
    DROP TABLE #tempResend
	 DROP TABLE #TempNonemployees
