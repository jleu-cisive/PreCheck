...
-- =============================================
CREATE PROCEDURE [dbo].[ARefchexIntegration_NotSentOrdersReport] 
AS
BEGIN

	SET NOCOUNT ON;
		select  distinct x.apno,x.appFirst ApplicantFirst, x.appLast applicantLast,x.appEmail as applicantEmail,x.appPhone applicantPhone, x.PersRefID,x.SectStat, x.SectSubStatusID, x.Web_Status,x.OrderId, x.DateOrdered, x.name, x.phone,x.Email,x.OperationType, x.CurrentOrderStatus,x.EmailCurrentStatus, x.EmailLastSentOn, x.EmailSentCount, x.SMSCurrentStatus, x.SMSLastSentOn, x.SMSSentCount, x.CreatedDate,x.response
		from
		(
		...
		where  lg.CreatedDate>='03/31/2021'
		and 
