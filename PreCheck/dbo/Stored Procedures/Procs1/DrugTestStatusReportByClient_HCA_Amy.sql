...
-- EXEC [DrugTestStatusReportByClient_HCA_Amy] 7519,7,1,null,null,null,null,null,0,0,0 
-- =============================================  
  
CREATE PROCEDURE [dbo].[DrugTestStatusReportByClient_HCA_Amy]  
(
	 @CLNO INT = 7519,  
	@NumberOfDays INT =  0,  
	@IsOneHR BIT =NULL,  
...
BEGIN  
	...
	*/
	...
	--SELECT @CutOffDate ='02/01/2022'   --select getdate()-21

	--  	declare @CLNO INT = 7519,  
	--@NumberOfDays INT = 21,  -- 0,  
	--@IsOneHR BIT =1  -- NULL,  
  ...
  drop table if exists #tmpResults
	...
	(  
		...
		LEFT JOIN dbo.OCHS_CandidateSchedule CS(NOLOCK)  ON C.OCHS_CandidateInfoID = CS.OCHS_CandidateID  
		LEFT JOIN dbo.OCHS_ResultDetails R(NOLOCK)  ON (CAST(C.OCHS_CandidateInfoID AS VARCHAR(25)) =  R.OrderIDOrApno) --and R.DateReceived<CS.CreatedDate 
	--	LEFT OUTER JOIN Precheck..Integration_OrderMgmt_Request iar (NOLOCK) ON C.APNO = iar.APNO
		...
		inner join #CLNOList cl on C.CLNO= cl.CLNO
		LEFT JOIN dbo.OCHS_CandidateSchedule CS(NOLOCK)  ON C.OCHS_CandidateInfoID = CS.OCHS_CandidateID  
		LEFT JOIN dbo.OCHS_ResultDetails R(NOLOCK)  ON (CAST(C.apno AS VARCHAR(25)) =  R.OrderIDOrApno) --and R.DateReceived<CS.CreatedDate 
	--	LEFT OUTER JOIN Precheck..Integration_OrderMgmt_Request iar (NOLOCK) ON C.APNO = iar.APNO
		WHERE ISNULL(orderIDOrApno,'0') <> '0' 
		  AND (CAST(ISNULL(LastUpdate,'1/1/1900') AS DATETIME) >= @CutOffDate OR @CutOffDate IS NULL)		 
		GROUP BY C.CLNO,C.LastName,C.FirstName,C.Middle,C.ClientIdent,CostCenter,ScheduledByID,TID,R.OrderIDOrApno,  
...
--	SELECT '#tmpResults' [#tmpResults],* FROM #tmpResults  
   ...
   drop table if exists #tmpMain
	...
	FROM  #tmpResults C  
	--inner join #CLNOList cl on C.CLNO= C.CLNO
	--LEFT JOIN Hevn..Facility F ON F.EmployerID=@CLNO --AND (C.CostCenter IS NULL OR C.CostCenter=F.FacilityNum) 
	LEFT JOIN dbo.refOCHS_CandidateInfoSchedule CSref ON C.ScheduledByID = CSref.refOCHS_CandidateInfoScheduleByID  
	LEFT OUTER JOIN Precheck..Integration_OrderMgmt_Request iar (NOLOCK) ON C.APNO = iar.APNO and iar.apno <>0
	--WHERE
	--( (C.CLNO IN (SELECT F1.FacilityCLNO FROM HEVN..Facility F1 WHERE ISNULL(F1.IsOneHR,0)=@IsOneHR AND ISNULL(F.ParentEmployerID,@CLNO) = @CLNO)  AND ISNULL(F.IsOneHR,0)=@IsOneHR)  
	--   OR (C.clno =@CLNO)  
