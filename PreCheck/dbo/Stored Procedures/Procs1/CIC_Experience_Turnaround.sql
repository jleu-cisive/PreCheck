...
-- EXEC [dbo].[CIC_Experience_Turnaround] '4','7519','01/05/2023','01/10/2023'
-- EXEC [dbo].[CIC_Experience_Turnaround] '4','7519','12/07/2022','12/11/2022'
...
CREATE PROC dbo.CIC_Experience_Turnaround
...
BEGIN
	...
	*/
	...
	DROP table if exists #tmp_Facility;
	DROP table if exists #Integration_OrderMgmt_Request
	...
	Create nonClustered index idx_tmp_ApplicantStage on #tmp_ApplicantStage(StagingOrderId)
	include ( FirstName,
		   LastName,
		   Email)
	...
	Create nonClustered index idx_tmp_vwStageOrder on #tmp_vwStageOrder(IntegrationRequestId)
	...
	from [Enterprise].[PreCheck].[vwClient]
	 ...
	 Create nonClustered index idx_tmp_vwClient on #tmp_vwClient(ClientId)
    ...
    SELECT app.StagingApplicantId,
		   ...
		   cast(0.00  as decimal(18,2)) AS [CandidateStart (Hours)],
           cast(0.00  as decimal(18,2))  AS [CandidatePortionCompleted(Hours)],
           cast(0.00  as decimal(18,2))  AS [Client Portion Completed (Hours)],
		   cast(0.00  as decimal(18,2))  AS [Overall CIC Turnaround (Hours)],
           app.SecurityTokenId,
           ord.FacilityId,
    ...
    INTO #tmp
     FROM #tmp_ApplicantStage app WITH (NOLOCK)
        ...
        INNER JOIN dbo.Appl with(nolock)
			on(app.ApplicantNumber = Appl.APNO)
		left JOIN hevn.dbo.Facility fac with(nolock)
			on(DeptCode = fac.FacilityNum
			and (@CLNO IS NULL
				OR fac.ParentEmployerID IN
					(
						SELECT value FROM fn_Split(@CLNO, ':')
					)
			)--13126--7519
			and isnull(fac.IsOneHR,0) = 1)
		where  Fac.FacilityCLNO is not null
...
ON ord.RequestID = a.IntegrationRequestId
  ...
  ON RR.ReviewRequestId = RRA.ReviewRequestId
	...
	UPDATE a
    SET a.[CandidateStart (Hours)] = ((DATEDIFF(MINUTE, a.[App Create Date], a.[Candidate Start Date])*1.0)/60.0)
	FROM #tmp a;

    UPDATE a
    SET a.[CandidatePortionCompleted(Hours)] = ((DATEDIFF(MINUTE, a.[Candidate Start Date], a.[Review Create Date])*1.0)/60.0)
    FROM #tmp a;
	...
	UPDATE a
    SET a.[Overall CIC Turnaround (Hours)] = ((DATEDIFF(Minute, a.app_createdate, a.ClientCertUpdated)*1.0)/60.0)
    FROM #tmp a;

    UPDATE a
    SET a.[Client Portion Completed (Hours)] =(((DATEDIFF(Minute, [Review Create Date], ClientCertUpdated))/60.0)*1.00)
    FROM #tmp a;
    ...
    SELECT a.AffiliateID,
		   ...
		   cast(a.[CandidateStart (Hours)]  as decimal(18,2)) as [CandidateStart (Hours)],
           cast(a.[CandidatePortionCompleted(Hours)]  as decimal(18,2)) as [CandidatePortionCompleted(Hours)] ,
           cast(a.[Client Portion Completed (Hours)] as decimal(18,2)) as [Client Portion Completed (Hours)],
		   a.R_ReleaseDate, 
		   a.ClientCertUpdated,
		   cast(a.[Overall CIC Turnaround (Hours)] as decimal(18,2)) as [Overall CIC Turnaround (Hours)]
		   into #temp
    FROM #tmp a
        INNER JOIN
        (
		...
		Select distinct AffiliateID,	AffiliateName,	ClientCLNO,	[Facility CLNO],	[Facility Name],[Facility Number],
			        RequisitionNumber,	[Order Number],	[First Name],	[Last Name],	Email,
					[Consent/Application Status],	[App Create Date],	[Candidate Start Date],
					[Review Create Date],ClientCertUpdated as [Review Completed Date],[Additional Info Requested Date],	
					[CandidateStart (Hours)],
					[CandidatePortionCompleted(Hours)],
					[Client Portion Completed (Hours)],
					[Overall CIC Turnaround (Hours)]
	from #temp
