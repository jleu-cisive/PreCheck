...
--[Bis3_MainReportSection]  3679154 
 

CREATE PROCEDURE [dbo].[Bis3_MainReportSection] @apno int AS

-- Detect if all sections need to be supressed

DECLARE @AliasList VARCHAR(max)
	SELECT APNO, ISNULL(Last,'') +', '+ ISNULL(First,'') +' '+ ISNULL(Middle,'') +' '+ ISNULL(Generation,'') AS QualifiedNames 
		INTO #tmpAliases
	FROM ApplAlias
	WHERE APNO = @apno and IsPrimaryName = 0 AND IsActive=1
