...
CREATE PROCEDURE [dbo].[DrugTestStatusReportByClient]          
(        
 @CLNO INT = 7519,          
 @NumberOfDays INT = 0,          
 @IsOneHR BIT = NULL,          
 ...
 */        
 ...
 IS NULL THEN 'N/A' ELSE iar.TransformedRequest.value('(Application/NewApplicant/RequisitionNumber)[1]','varchar(50)') END) AS RequisitionNumber        
  FROM [PreCheck].[dbo].[OCHS_CandidateInfo] C(NOLOCK)            
  LEFT JOIN dbo.OCHS_CandidateSchedule CS(NOLOCK)  ON C.OCHS_CandidateInfoID = CS.OCHS_CandidateID          
