-- Alter Procedure Crim_Automate_Print_1
...
Execution: EXEC [dbo].[Crim_Automate_Print_1] 4808203, 'CrimOnDB','O'
*/
CREATE PROCEDURE [dbo].[Crim_Automate_Print_1] 
...
-- The differences are the number of parameters and some of criterias are different
	...
	INNER JOIN (SELECT DISTINCT APNO, COUNT(1) AS AliasCount FROM #tmpCrimsSentToVendor(NOLOCK) GROUP BY APNO) AS Y ON T.APNO = Y.APNO

	--SELECT * FROM #tmpAliasCount

	SELECT	DISTINCT APNO, ISNULL([Last],'') AS [Last], ISNULL([First],'') AS [First], ISNULL(Middle,'') AS Middle, ISNULL(Generation,'') AS Generation , 
	...
	GROUP BY APNO, Last, First, Middle, Generation, AliasCount
	ORDER BY APNO
  ...
  BEGIN
		SELECT  --A.Last, A.First, A.Middle,
				--A.APNO, ISNULL(TA.AliasCount,0) AliasCount,
