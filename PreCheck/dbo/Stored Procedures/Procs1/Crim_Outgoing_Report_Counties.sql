-- Alter Procedure Crim_Outgoing_Report_Counties
/*
Modifed By: Deepak Vodethela
Modified Date: 02/23/2017
Description: This SP is used in "Iris_outgoing_counties.rpt". Modified to fit in the new requirement of Alias List Display on report using the new tables.
Execution: 
-- WEB SERVICE
EXEC [dbo].[Crim_Outgoing_Report_Counties] '4707786','dvodethe','O'

-- All Other
EXEC [dbo].[Crim_Outgoing_Report_Counties] '6363683','CrimWeb','M'

*/

CREATE PROCEDURE [dbo].[Crim_Outgoing_Report_Counties] 
...
-- The differences are the number of parameters and some of criterias are different
	...
	CREATE CLUSTERED INDEX IX_tmpCrimsSentToVendor_01 ON #tmpCrimsSentToVendor([ApplAliasID],APNO)
	CREATE CLUSTERED INDEX IX_tmpCrims_01 ON #tmpCrims([ApplAliasID], APNO)
	...
	FROM Crim(NOLOCK) 
	WHERE [STATUS] = @newprintnumber
	  AND [Clear] IN ('M','O')

	CREATE CLUSTERED INDEX IX_tmpCrimsForStatus_01 ON #tmpCrimsForStatus(CrimID, APNO)
	...
	CREATE CLUSTERED INDEX IX_tmpAliasCount_01 ON #tmpAliasCount([ApplAliasID], APNO)
	...
	CREATE CLUSTERED INDEX IX_tmpAliasesSentToVendor_01 ON #tmpAliasesSentToVendor(CrimID, APNO)
	...
	--CREATE CLUSTERED INDEX IX_tmpSelectedAliases_01 ON #tmpSelectedAliases(CrimID, APNO)
	...
	BEGIN
		SELECT	A.[Last] , A.[First] , A.Middle
				...
				, IR.R_VendorNotes , C.County AS Bis_Crim_County , A.APNO
				, CO.A_County , CO.State AS crimstate, CO.country, C.CrimID
		...
		INNER JOIN dbo.TblCounties AS CO WITH (NOLOCK) ON C.CNTY_NO = CO.CNTY_NO
