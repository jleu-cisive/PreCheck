...
-- =============================================
CREATE PROCEDURE [dbo].[FetchOnHoldApps] 
...
--This recordset has a list of qualified emails per client - that has reports OnHold.
 ...
 --Select distinct 'Kiranmiryala@precheck.com' Email,clno
 From
 (
 ...
 SELECT ((case when isnull(CAMEmail,'') is null then Email else Email + ',' + CAMEmail end) + ',CarlaBingham@precheck.com') email,clno 
 ...
 SELECT Distinct APNO,Name,apdate,clno,clientapno into #tempFinalApps
 From
 (
 ...
 FROM  #tmpemailsWithCAM
  ...
  Select a.APNO ,ApDate ,(a.last + ', ' +  a.first) as Name,  Isnull(ClientCertReceived,'No') ClientCertReceived Into #temp1	
	 	FROM Appl a  WITH (NOLOCK)   left join ClientCertification WITH (NOLOCK) on  a.Apno = ClientCertification.Apno
	where a.ApStatus = 'M'  and a.CreatedDate > '9/1/2015' and a.EnteredVia not in( 'StuWeb','System') and a.Inuse is null and a.CLNO not in (3468,2135)

	

	Select a.APNO ,case  when Isnull(ClientCertReceived,'No') = 'No' then a.Name +' (Certification Missing) ' else a.Name  end as Name,a.apdate,a.clno,clientapno 
	from #tempFinalApps a left join #temp1 t1 on a.APNO = t1.APNO

 DROP TABLE #tmpemailsWithCAM
 DROP TABLE #tmpOnHoldNotification
  DROP TABLE #tempFinalApps
    DROP TABLE #temp1
