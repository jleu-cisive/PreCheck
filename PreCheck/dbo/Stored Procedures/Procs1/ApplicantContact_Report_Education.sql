...
CREATE PROCEDURE [dbo].[ApplicantContact_Report_Education]
...
AS
	...
	SELECT
		...
		(SELECT e.School FROM Educat AS e WHERE e.EducatID = ac.SectionUniqueID) AS 'Component Description',
		(SELECT ss.Description FROM Educat AS e INNER JOIN dbo.SectStat ss ON e.SectStat=ss.Code WHERE e.EducatID = ac.SectionUniqueID) AS 'Status', -- Modified by Humera Ahmed for HDT#52384
