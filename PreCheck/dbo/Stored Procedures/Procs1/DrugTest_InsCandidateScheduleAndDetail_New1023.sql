...
Create procedure [dbo].[DrugTest_InsCandidateScheduleAndDetail_New1023](@starttime datetime = null,@endtime datetime= null)
as
BEGIN
...
select 
	...
	case when isnull(cci1.Value,cci3.Value)='True' then DateAdd(day,@defaultDrugTestExpirationDays,CONVERT(date, CI.CreatedDate)) 
		 ELSE
			case when IsNumeric(RTRIM(LTRIM(isnull(cci.Value,cci2.value)))) = 0 then [dbo].[fnGetNthBusinessEndDateWithHolidays](CONVERT(date, CI.CreatedDate),@defaultDrugTestExpirationDays) else[dbo].[fnGetNthBusinessEndDateWithHolidays](CONVERT(date,CI.CreatedDate),cast(isnull(cci.Value,cci2.value) as int)) end 
...
inner join Enterprise.Staging.vwStageOrder os (nolock) on cast(os.DrugTestServiceNumber as int) = IsNull(ci.OCHS_CandidateInfoID,0)
...
where 
 ...
 and ci.CreatedDate > @cutoffDate and os.[CreateDate] > @cutoffDate
	 and ((ci.CreatedDate BETWEEN @StartTime AND @EndTime) or (@starttime is  null and @endtime is  null))
