...
--First_Attempt_Verification_Details_For_Employment_kiran '01/05/2015', '01/10/2015'


CREATE Procedure [dbo].[First_Attempt_Verification_Details_For_Employment_kiran]
(
@StartDate DateTime = '2015-02-10 10:00', 
...
and (min(changedate)> CONVERT(DATETIME, @StartDate, 102))
 and min(changedate)< = dateadd(s,-1,dateadd(d,1,@EndDate)) 
and oldvalue in ('9', '0') and NewValue  = '5'  --and count(id)<2
order by id

--select @StartDate,@EndDate, CONVERT(DATETIME, @StartDate, 102),dateadd(s,-1,dateadd(d,1,@EndDate))
select * from #temp11 where UserID = 'ABigas'
...
--group by e.id,e.userid  --having   count(e.id)<2
 select * from #temp where UserID = 'ABigas'
...
and ((changedate)>  CONVERT(DATETIME, @StartDate, 102)) 
and (changedate)< =  dateadd(s,-1,dateadd(d,1,@EndDate)) 
and oldvalue = '0'

---select * from #temp12 --where UserID = 'ABigas'
...
SELECT distinct UserID,id into #tempChangeLog3
FROM dbo.ChangeLog 
