...
CREATE PROCEDURE [dbo].[Education_Verification_Status_QReport_test] 

@StartDate DateTime = '08/29/2011', 
@EndDate DateTime = '08/29/2011'
...
(Select count(1) From #tmp1 A where web_Updated is not null and A.Investigator = T.Investigator) [Verifications - Updated WebStatus],
	  (Select count(1) From dbo.Educat B (NoLock)  where sectstat = '4'and Last_Worked>= @StartDate and Last_Worked< = dateadd(s,-1,dateadd(d,1,@EndDate))and isnull(B.Investigator,'') = isnull(T.Investigator,'')) [VERIFIED],
	   (Select count(1) From dbo.Educat C (NoLock)  where sectstat = '5' and Last_Worked>= @StartDate and Last_Worked< = dateadd(s,-1,dateadd(d,1,@EndDate)) and isnull(C.Investigator,'') = isnull(T.Investigator,'')) [VERIFIED/SEE ATTACHED],
	   (Select count(1) From dbo.Educat D (NoLock)  where sectstat = '6' and Last_Worked>= @StartDate and Last_Worked< = dateadd(s,-1,dateadd(d,1,@EndDate)) and isnull(D.Investigator,'') = isnull(T.Investigator,'')) [UNVERIFIED/SEE ATTACHED],
	   (Select count(1) From dbo.Educat E (NoLock)  where sectstat = '8' and Last_Worked>= @StartDate and Last_Worked< = dateadd(s,-1,dateadd(d,1,@EndDate)) and isnull(E.Investigator,'') = isnull(T.Investigator,'')) [SEE ATTACHED],
	   (Select count(1) From dbo.Educat F (NoLock)  where sectstat = '7' and Last_Worked>= @StartDate and Last_Worked< = dateadd(s,-1,dateadd(d,1,@EndDate)) and isnull(F.Investigator,'') = isnull(T.Investigator,'')) [ALERT/SEE ATTACHED],
	   (Select count(1) From dbo.Educat G (NoLock)  where sectstat = '3' and Last_Worked>= @StartDate and Last_Worked< = dateadd(s,-1,dateadd(d,1,@EndDate)) and isnull(G.Investigator,'') = isnull(T.Investigator,'')) [COMPLETE/SEE ATTACHED],
	   (Select count(1) From dbo.Educat H( NoLock)  where sectstat = '2' and Last_Worked>= @StartDate and Last_Worked< = dateadd(s,-1,dateadd(d,1,@EndDate)) and isnull(H.Investigator,'') = isnull(T.Investigator,'')) [COMPLETE],
	   (Select count(1) From dbo.Educat I (NoLock)  where sectstat = '9' and Last_Worked>= @StartDate and Last_Worked< = dateadd(s,-1,dateadd(d,1,@EndDate)) and isnull(I.Investigator,'') = isnull(T.Investigator,'')) [Pending - Assigned],
	   (Select count(1) From dbo.Educat  (NoLock) where sectstat = '9') [Pending - Overall]
From #tmp1 T
Group By T.Investigator
UNION ALL
Select 'Totals' Investigator, 
--(Select count(1) From dbo.Educat J (NoLock) where  (InvestigatorAssigned>= @StartDate and InvestigatorAssigned< = dateadd(s,-1,dateadd(d,1,@EndDate))) ) 
 0 [Verifications Assigned],
--count(1) [Verifications Assigned],
 --(Select count(1) From #tmp1 A where web_Updated is not null ) [Verifications - Updated WebStatus],
		(Select count(1) From dbo.Educat A (NoLock)  where web_Updated is not null and Last_Worked>= @StartDate and Last_Worked< = dateadd(s,-1,dateadd(d,1,@EndDate))) [VERIFIED],
