...
-- Modified by radhika dereddy on 08/18/2020 to add PackageOrdered,Apstatus, InProgressReviewed, EnteredVia, AffiliateName   
 --Modified by James Norton on 08/10/2021 to removed join dbo.ApplAdditionalData-  data not used. 
 --Modified by Mainak Bhadra on 10/18/2021 to add AffiliateId for ticket #67224
 --Execution: EXEC [dbo].[CAM_Activity_Report_Details]  NULL, '10:4'   

-- =========================================================================================  
  
CREATE PROCEDURE [dbo].[CAM_Activity_Report_Details]  
@CAM varchar(8) = NULL,
...
AS  
 ...
 IF OBJECT_ID('tempdb..#tmpFinalCAMActivity') IS NOT NULL  
  DROP TABLE #tmpFinalCAMActivity  
 ...
 AND (@AffiliateId IS NULL OR RA.AffiliateId IN (SELECT value FROM fn_Split(@AffiliateId,':')))--code added by Mainak for ticket id -67224
  ...
  --select * from #APNOList   
    ...
    (SELECT Last_Updated, APNO FROM dbo.Appl WITH(NOLOCK) WHERE apno IN (SELECT apno FROM #APNOList) )  
       ...
       UNION ALL  
       (SELECT Last_Updated, APNO FROM dbo.Credit WITH(NOLOCK) WHERE  apno IN (SELECT apno FROM #APNOList) and SectStat NOT IN ('0','9') and reptype IN ('S', 'C')  AND Last_Updated IS NOT null)   
       UNION ALL  
...
-- Get CAM Details  
 ...
 --select * from #CAMActivityDetails  
  ...
  --SELECT * FROM #tempCAMDetailFinal  
 -- Get Report Fields  
