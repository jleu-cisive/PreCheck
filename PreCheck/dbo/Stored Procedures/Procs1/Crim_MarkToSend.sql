...
-- Edited date:	03/24/2017
-- Description:	As part of Alias Logic Re-Write project, alias sent will be added to the private notes
...
--[Crim_MarkToSend] 'WEB SERVICE','AutoOrdr'


CREATE PROCEDURE [dbo].[Crim_MarkToSend]    
			...
			If @deliverymethod = 'InHouse' 
					Insert into #tmp
						(crimid,vendorid,cnty_no)
							SELECT C.crimid,c.vendorid,c.cnty_no FROM appl A 
								INNER JOIN crim C ON A.apno = C.apno
								INNER JOIN iris_researchers IR ON C.vendorid = IR.r_id    WHERE
...
--								AND (C.batchnumber IS NULL)
--								AND (DATEDIFF(mi, C.crimenteredtime, GETDATE()) >= 1)
--								AND (UPPER(IR.r_delivery) = UPPER(@deliverymethod)) AND 
--							case when @deliverymethod = 'OnlineDB' then  	(isnull(IR.password,'') <> '') else (isnull(IR.password,'') = '')
...
Select * from #tmp
		...
		--update records from temp table
			...
			SET [clear] = case when deliverymethod = 'Integration'  then 'M' else  'O' end,
				...
				[IrisFlag] = 1,
				[IrisOrdered]=getdate()
			from crim cc  join #tmp tmp on cc.crimid = tmp.crimid
			--WHERE cc.crimid IN (SELECT crimid FROM #tmp);

	select @OrderDate,tmp.cnum,tmp.bnum
	from crim cc  join #tmp tmp on cc.crimid = tmp.crimid
...
INSERT INTO [dbo].[Crim_ResendLog]
   ...
   from crim WHERE        (IsHidden = 0) AND (Clear = 'e'))

   -- Deepak:11/03/2020 --> Resetting (InUseByIntegration, Ordered) column for the service to pick it up in next run.
	update dbo.Crim
	set readytosend = 1,
		batchnumber = Null,
		dbo.Crim.InUseByIntegration = NULL,
		Ordered = NULL
	WHERE  (IsHidden = 0) AND (Clear = 'e')


	CREATE TABLE #tmp2(tmp2ID int identity(1,1) primary key not null, crimid int)
	Insert into #tmp2(crimid)
	SELECT DISTINCT C.crimid FROM appl A 
            INNER JOIN crim C ON A.apno = C.apno
            INNER JOIN iris_researchers IR ON C.vendorid = IR.r_id
        WHERE
            (UPPER(A.apstatus) IN ('P','W'))
			 AND ((A.inuse IS NULL) OR (A.inuse = 'ChkAlias'))
            AND (C.readytosend = 1)
...
Print '99 - crim table' 
        SELECT C.crimid,clear,[batchnumber],[IrisOrdered]  FROM appl A 
            INNER JOIN crim C ON A.apno = C.apno
            INNER JOIN iris_researchers IR ON C.vendorid = IR.r_id
        WHERE
            (UPPER(A.apstatus) IN ('P','W'))
