...
CREATE PROCEDURE [dbo].[FormSchoolReportAppListByViewTest] 
  @ClientID varchar(10) = '',
  @Program varchar(10) = '',
  @SSN varchar(11) = '',
  @First varchar(20)= '',
...
begin 
		...
		begin
			set @FirstDate =  CONVERT(varchar,dateadd(year,-3,getdate()),1)--'01/01/2007'
			set @EndDate =   CONVERT(varchar, getdate()+1, 1)
		end

--set @FirstDate =CONVERT(varchar,dateadd(year,-3,getdate()),1)
----('1/1/'+ Convert(varchar,(Year(CONVERT(varchar,dateadd(year,-2,getdate()),1)))))  
--set @EndDate =   CONVERT(varchar, getdate()+1, 1)
end
