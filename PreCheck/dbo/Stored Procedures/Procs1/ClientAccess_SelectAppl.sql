...
--EXEC [ClientAccess_SelectAppl] 2915659,'7926',9840

CREATE PROCEDURE [dbo].[ClientAccess_SelectAppl] --2915659,'7926',9840
@Apno int,
@SSN Varchar(20),
@CLNO int
AS
...
SELECT a.APNO, a.ApStatus, a.UserID, a.CLNO
	...
	,case when len(aa.StatusID) > 0 then aa.Address1 +','+ aa.Address2 else isnull(a.Addr_Num,'') + ' ' + isnull(a.Addr_Apt,'') + ' ' + isnull(a.Addr_Dir,'') + ' ' + a.Addr_Street  + ' ' +  isnull(Addr_StType,'') end as Addr_Street
	,aa.Address2
	...
	,ISNULL(aa.StatusID,0) as StatusID
, aa.ClientEmail
,ISNULL(aa.ApplicantEmail, (case when a.Email = '' then  NULL else a.Email  End)) as ApplicantEmail 
