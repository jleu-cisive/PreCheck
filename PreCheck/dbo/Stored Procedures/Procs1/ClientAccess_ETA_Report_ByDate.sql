...
-- EXEC [ClientAccess_ETA_Report_ByDate] 12143, '08/07/2018', '08/07/2018'
-- EXEC [ClientAccess_ETA_Report_ByDate] 7519, '01/30/2019', '01/31/2019'
-- EXEC [ClientAccess_ETA_Report_ByDate] 3115,'06/01/2018','06/21/2018'
-- =============================================
-- Author:		Dongmei He
...
-- =============================================================================
CREATE PROCEDURE [dbo].[ClientAccess_ETA_Report_ByDate]
 @CLNO  INT,
 @StartDate	  Date,
 @EndDate     Date
...
As
   ...
   SELECT @count = count(CLNO) FROM Client WHERE CLNO =@clno OR WebOrderParentCLNO = @clno
	...
	INNER JOIN ApplSectionsETA AS ETA(NOLOCK) ON A.APNO	 = ETA.Apno	
	INNER JOIN ClientCertification C(NOLOCK) ON ETA.APNO = C.APNO
	...
	BEGIN
		...
		WHERE A.ApStatus NOT IN ('F','M')
		  AND A.OrigCompDate IS NULL
		  AND C.IsInactive = 0
		  AND A.Clno IN (SELECT CLNO FROM Client(NOLOCK) WHERE Clno = @CLNO)
	END
