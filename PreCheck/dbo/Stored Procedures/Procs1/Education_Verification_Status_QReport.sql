...
/*********************************************************************************  
EXEC [dbo].[Education_Verification_Status_QReport_Deepak] '08/01/2015', '08/24/2015'  
EXEC [dbo].[Education_Verification_Status_QReport] '01/08/2020', '01/08/2020'  
-- Modified by AmyLiu on 09/03/2020 for phase3 of project:IntranetModule: Status-Substatus  
-- exec dbo.Education_Verification_Status_QReport '09/03/2020','09/03/2020'  
********************************************************************************/  
 CREATE PROCEDURE [dbo].[Education_Verification_Status_QReport]   
...
--Set @StartDate = '09/03/2020'  
--Set @EndDate = '09/03/2020'  
...
Select distinct UserID into #tempUsers from Users (NoLock) Where Educat = 1 and Disabled = 0 --(Commented by Radhika Dereddy on 04/07/2014 as per Milton's request to have all investiagtors incuded irrrespective of their activity each day)  
order by UserID  
...
--    (Select count(1) From dbo.#tempChangeLog H( NoLock)  where Newvalue = '2'  and isnull(H.UserID,'') = isnull(T.UserID,'')) [COMPLETE],  
 (Select count(1) From dbo.#tempChangeLog X (NoLock)  where Newvalue = 'A'  and isnull(X.UserID,'') = isnull(T.UserID,'')) [EDUCATIONAL INSTITUTION CLOSED],  
    ...
    (Select count(1) From dbo.#tempChangeLog I (NoLock)  where Newvalue = 'C'  and isnull(I.UserID,'') = isnull(T.UserID,'')) [ALERT],  
    (Select count(1) From dbo.#tempChangeLog I (NoLock)  where Newvalue = 'U'  and isnull(I.UserID,'') = isnull(T.UserID,'')) [UNVERIFIED],  
    (Select count(1) From dbo.#tempChangeLog I (NoLock)  where Newvalue = 'R'  and isnull(I.UserID,'') = isnull(T.UserID,'')) [ComplicanceReinvestigation],  
...
--    (Select count(1) From dbo.Educat H (NoLock)  where sectstat = '2' and Last_Worked>= @StartDate and Last_Worked< = dateadd(s,-1,dateadd(d,1,@EndDate))) [COMPLETE],  
    ...
    (Select count(1) From dbo.Educat I (NoLock)  where sectstat = 'R' and Last_Worked>= @StartDate and Last_Worked< = dateadd(s,-1,dateadd(d,1,@EndDate)) ) [UNVERIFIED],  
   -- (Select count(1) From dbo.Educat  (NoLock) where sectstat = '9') [Pending - Overall]  
 (Select count(1) From dbo.Educat E (NoLock) inner JOin Appl A (NoLock) on E.APNO = A.APNO  where sectstat = '9' and  A.ApStatus not in ('F') ) [Pending - Overall]  
