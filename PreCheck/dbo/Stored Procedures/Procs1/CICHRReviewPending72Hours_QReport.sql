...
-- Description:	Please create a new Q report Called CIC HR Review Pending 72 Hours 
-- This report should be based on the structure of the CIC Pending Invitations QReport but will only include the items that are Pending HR Review from the client for more than 72 hours. 
-- Created by modifying the stored procedure CICHRReviewPending48Hours_QReport
-- Exec [dbo].[CICHRReviewPending72Hours_QReport]
-- =============================================
CREATE PROCEDURE [dbo].[CICHRReviewPending72Hours_QReport] 
...
BEGIN
	...
	WHERE 	
		...
		AND RR.IsComplete = 0
		AND DATEDIFF(HOUR, RR.CreateDate, GETDATE()) > 72
