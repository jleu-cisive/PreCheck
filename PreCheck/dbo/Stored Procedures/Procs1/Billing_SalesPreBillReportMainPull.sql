...
-- =============================================
CREATE PROCEDURE [dbo].[Billing_SalesPreBillReportMainPull]
	-- Add the parameters for the stored procedure here
	@CutOffDate datetime,@BillingCycle varchar(2),@SalesRep varchar(20)
AS
BEGIN
...
--        (refBC.BILLingCYCLE = @BillingCycle) )  ) AND
--Client.SalesPersonUserID = @SalesRep AND  
--(CLIENT.LastInvAmount = 0 or CLIENT.LastInvAmount is null) AND not CLIENT.SalesPersonuserID is null AND CLIENT.IsInactive = 0 AND CLIENT.nonClient = 0 AND --
--CLIENT.CLNO not in (SELECT CLNO FROM InvMaster WITH (NOLOCK) Group by clno having SUM(Sale) > 0)
--
--
--ORDER BY APPL.CLNO ,
...
AND (refBC.BILLingCYCLE = @BillingCycle)  AND 
Client.SalesPersonUserID = @SalesRep AND  
(CLIENT.LastInvAmount = 0 or CLIENT.LastInvAmount is null) AND not CLIENT.SalesPersonuserID is null AND CLIENT.IsInactive = 0 AND CLIENT.nonClient = 0 AND 
CLIENT.CLNO not in (SELECT CLNO FROM InvMaster WITH (NOLOCK) Group by clno having SUM(Sale) > 0)
