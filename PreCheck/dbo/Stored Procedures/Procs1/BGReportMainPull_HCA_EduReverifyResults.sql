...
-- Execution: EXEC [BGReportMainPull_HCA_EduReverifyResults] 7519, '11/01/2019','03/10/2020',1  
-- =============================================  
  
CREATE PROCEDURE [dbo].[BGReportMainPull_HCA_EduReverifyResults]  
...
Select apno ,max(BackgroundReportid) as   BackgroundReportid from BackgroundReports.dbo.BackgroundReport where apno in (  Select apno from appl where clno = 15989)
group by apno
  ...
  INSERT INTO #tmp  
  SELECT DISTINCT idtable.apno as FolderID,7519 AS Clno, br.backgroundreportid AS ReportID,idtable.employeeNumber, isnull(ClientFacilityGroup,'NonGrouped') as ClientFacilityGroup --schapyala returning emptystring for facilitygroup as this is not used for BG purposes - 05/04/2015  
  FROM (  
 ...
 INNER JOIN (SELECT facilityid,ClientFacilityGroup   
   ...
   WHERE a.apstatus = 'F' and a.clno = 15989 
   and A.Apno in ( select [Report Number] from [dbo].[HCA_EDU_Verifications]) 
   ...
   --and idTable.apno in (select APNO from appl where clno = 15989    
   and   
 idtable.Apno not in (  
Select apno from BackgroundReports.dbo.BackgroundReport where backgroundreportid in (  
select Reportid from ReportUploadLog where ReportUploadVolumeid in (  
...
--)
  ...
  SELECT Top 1000 * FROM #tmp t ORDER BY t.FolderID ASC  
