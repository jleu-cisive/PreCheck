...
-- Request: Add AttnTo field to the Invoice Detail and Summary
--sp_helptext [Billing_ClientReportInvSummary_ByClientGroup_AttentionTo]
--Exec [Billing_ClientReportInvSummary_ByClientGroup_AttentionTo] 12444,null,null,10
--Exec [Billing_ClientReportInvSummary_ByClientGroup_AttentionTo] 12549
*/
CREATE PROCEDURE [dbo].[Billing_ClientReportInvSummary_ByClientGroup_AttentionTo] 
@CLNO int,
...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	IF (isnull(@ClientGroupCode,0)  in (2,3)  ) --only include Department and GL Account for HCA Physician recruitment and ParallonWFS
			Select Distinct 'Summary' FileType, (case when isnull(@ClientGroupCode,0) = 2 and isnull(Department,'') = '' then right(Client,3) else Department end) Department,
				...
				CompDate [PreCheck Request Completion Date],Client [Precheck CLNO],SubTotal,Tax, (Subtotal + Tax) Total, AttentionTo
	...
	ELSE
			Select Distinct 'Summary' FileType, Case When (len(Department)=5 and len([Process Level]) < 5) then Department else (case when isnull([Process Level],'')='' then Department else [Process Level] end) end [Process Level], [Last Name],[First Name],
				[Middle Name],invoicenumber [PreCheck Invoice Number],InvDate [PreCheck Invoice Date], t1.APNO [PreCheck RequestID],CompDate [PreCheck Request Completion Date],Client [Precheck CLNO],SubTotal,Tax, (Subtotal + Tax) Total,
				AttentionTo
			from #tmpInv t1 inner join #tmpInv2 t2 on t1.APNO = t2.APNO
	...
	--BEGIN	
		...
		Select 'Detail' FileType, Department,[Process Level],[GLAccount#],[Last Name],[First Name],[Middle Name],invoicenumber [PreCheck Invoice Number],InvDate [PreCheck Invoice Date],APNO [PreCheck RequestID],CompDate [PreCheck Request Completion Date],
			Client [Precheck CLNO],Replace(Description,',',' ') Description,[AddOn Fee],AttentionTo,Amount into #tmpDetail From
			(
				...
				[Description],[AddOn Fee], AttentionTo,Amount
			From #tmpInv
	...
	IF (isnull(@ClientGroupCode,0)  in (2,3) ) --only include Department and GL Account for HCA Physician recruitment and ParallonWFS
			...
			Select FileType, (case when isnull(@ClientGroupCode,0) = 2 and isnull(Department,'') = '' then right([Precheck CLNO],3) else Department end) Department,
				...
				[Precheck CLNO],Replace(Description,',',' ') Description,[AddOn Fee],AttentionTo,Amount From #tmpDetail 
