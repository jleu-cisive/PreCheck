-- Create Procedure GetPreCheckCountyFromZipCrimLead_new
/*
EXEC [dbo].[GetPreCheckCountyFromZipCrimLead] 'ABUSE', NULL, 'OH', NULL
EXEC [dbo].[GetPreCheckCountyFromZipCrimLead] 'FELMSD', NULL, 'SC', 'STATEWIDE'
...
EXEC [dbo].[GetPreCheckCountyFromZipCrimLead] 'FELMSD', NULL, 'CO', 'YUMA'
EXEC [dbo].[GetPreCheckCountyFromZipCrimLead_new] 'FELMSD', '24510', 'AZ', 'YUMA'
*/
CREATE PROCEDURE [dbo].[GetPreCheckCountyFromZipCrimLead_new]
...
AS
	...
	SELECT CNTY_NO, CNTY_NO, LTRIM(RTRIM([State])), 'ABUSE' FROM dbo.TblCounties WHERE dbo.TblCounties.CNTY_NO IN (5295)	
	UNION ALL
	SELECT CNTY_NO, CNTY_NO, LTRIM(RTRIM([State])), 'OFFNDR' FROM dbo.TblCounties WHERE cnty_no IN (5098)
	...
	IF (@leadType = 'BNKRPT')
	BEGIN
		SELECT c.CNTY_NO FROM dbo.TblCounties c WHERE c.A_County = 'FEDERAL BANKRUPTCY'
		RETURN 0
	END 
	
	IF (@leadType = 'FEDCRM' AND @subType = 'NATIONWIDE')
	BEGIN
		SELECT c.CNTY_NO FROM dbo.TblCounties c WHERE c.A_County = 'DISTRICT COURT FEDERAL'
		RETURN 0
	END
	...
	BEGIN
		...
		BEGIN
			...
			IF @cnty is null
				SELECT @cnty = CNTY_NO FROM dbo.TblCounties c
		...
		IF @subType = 'TX DPS'
		BEGIN
			SELECT CNTY_NO FROM dbo.TblCounties c
		...
		BEGIN
			...
			IF (@subType = 'SC')
			BEGIN
				SELECT coalesce(sc.cntyToOrder, c.CNTY_NO) AS CNTY_NO FROM dbo.TblCounties c
				LEFT JOIN @specialCounties sc ON sc.cnty_no = c.CNTY_NO
				WHERE c.refCountyTypeID = 2 
				AND c.[State] = @subType
