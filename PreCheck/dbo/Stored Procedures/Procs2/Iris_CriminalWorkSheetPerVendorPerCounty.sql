--[dbo].[Iris_CriminalWorkSheetPerVendorPerCounty] 1819258, 2719

--exec [Iris_CriminalWorkSheetPerVendorPerCounty04062017] 1819258, 2719

--exec [Iris_CriminalWorkSheetPerVendorPerCounty_forEXCEL] 1819258, 2719

CREATE PROCEDURE [dbo].[Iris_CriminalWorkSheetPerVendorPerCounty] 
@VendorID int,
@County int
...
dbo.Crim.CrimID as CrimID, 
    ...
    --as CkName,
	case when isnull(AA.AliasName, '') = '' then isnull(dbo.Appl.Last,'') + ', ' + isnull(dbo.Appl.First, '') + ' ' + ISNULL(dbo.Appl.Middle, '') ELSE AA.AliasName END  as CKName,
	...
	CONCAT('Client ID ',cl.clno,', Affiliate ID ',ISNULL(cl.affiliateid,'0'),',',ISNULL(dbo.Crim.CRIM_SpecialInstr, '')) as CRIM_SpecialInstr,
...
INNER JOIN Client cl WITH (NOLOCK) ON dbo.Appl.clno=cl.clno
LEFT JOIN 
	(SELECT t.SectionKeyID
			,STUFF((SELECT DISTINCT ' / ' + isnull(A.Last, '') + ', ' +  isnull(A.First, '') + ' ' + isnull(A.Middle, '')
                    ...
                    WHERE s.SectionKeyID = t.SectionKeyID AND S.IsActive = 1 AND S.ApplSectionID = 5
					FOR XML PATH(''),TYPE).value('.','VARCHAR(MAX)'),1,2,'') AS AliasName
 FROM ApplAlias_Sections t 
GROUP BY t.SectionKeyID
	) AA
	ON AA.SectionKeyID = crim.CrimID

-- INNER JOIN dbo.ApplAlias_Sections S ON S.SectionKeyId = dbo.crim.CrimID and S.IsActive = 1
 WHERE     (dbo.Crim.VendorID = @VendorID) and (dbo.Crim.Cnty_no = @County) and 
(dbo.Crim.Clear in('O','W'))
--and (dbo.Appl.InUse is null) 
and crim.ishidden = 0  order by Ordered, AA.AliasName
