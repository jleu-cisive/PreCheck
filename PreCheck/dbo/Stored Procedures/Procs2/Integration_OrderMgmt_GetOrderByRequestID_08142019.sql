	...
	CREATE PROCEDURE [dbo].[Integration_OrderMgmt_GetOrderByRequestID_08142019]   
	...
	BEGIN  
	 ...
	 if (@req2 is null)  
		...
		select top 1 CLNO,@req1 as RawRequest,@req2 as Request,FacilityClno,refUserActionID,1 as NeedsTransform,IsNull(Apno,'') as Apno from dbo.Integration_OrderMgmt_Request where RequestID = @RequestID 
	 ...
	 -- select top 1 CLNO,@req1 as Request,refUserActionID,1 as NeedsTransform,IsNull(Apno,'') as Apno from dbo.Integration_OrderMgmt_Request where RequestID = @RequestID  
		...
		select top 1 CLNO,@req1 as RawRequest,@req2 as Request,FacilityClno,refUserActionID,0 as NeedsTransform,IsNull(Apno,'') as Apno from dbo.Integration_OrderMgmt_Request where RequestID = @RequestID 
