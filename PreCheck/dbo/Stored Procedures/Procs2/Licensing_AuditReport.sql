...
-- Ticket no. - 5505
-- exec  [dbo].[Licensing_AuditReport]  '02/25/2021','02/25/2021'
-- =============================================
CREATE PROCEDURE [dbo].[Licensing_AuditReport] 
	@StartDate DateTime , 
	@EndDate DateTime 
AS
BEGIN
	...
	--select  tablename from ChangeLog with(nolock) where tablename ='ProfLic.pub_notes'
...
SELECT *, 't1' src INTO #temp1
  FROM ( 
	   ...
	   LEFT OUTER JOIN ( SELECT sectionkeyid FROM  dbo.Integration_Verification_SourceCode with (nolock) 
		...
		WHERE c.[TableName] = 'ProfLic.sectstat'  
...
SELECT  * , 't2' src into #temp2
	FROM ( SELECT ROW_NUMBER() over (partition by c.[ID], Convert(date, c.[ChangeDate]) order by c.[ChangeDate] desc) rn,c.[ID],
		  ...
		  WHERE c.[TableName] = 'ProfLic.web_status' 
		  --and c.[UserID] like  @UserId + '%' 
		  and c.ChangeDate between @StartDate and @EndDate
     ) a
WHERE a.[rn] = 1
...
SELECT * , 't3' src into #temp3
  FROM (SELECT ROW_NUMBER() over (partition by c.[ID], Convert(date, c.[ChangeDate]) order by c.[ChangeDate] desc) rn,c.[ID],
         ...
         WHERE ( c.[TableName] = 'ProfLic.priv_notes' or c.TableName = 'ProfLic.pub_notes')
		 --and c.[UserID] like  @UserId + '%' 
		 and c.ChangeDate between @StartDate and @EndDate
       ) a
WHERE a.[rn] = 1
...
from 
	...
	SELECT Apno, Employer, Web_Status, UserId, /*Id,*/ Status, SubStatus, ChangeDate,/* DuplicateRecords, WorkNumber,  DuplicateWorkNumber,*/ EmplEfforts as Efforts  
	FROM
	(
