...
Create PROCEDURE [dbo].[PrecheckFramework_UpdateFromStaging_02202019]
...
BEGIN  
 ...
 --      and stg.Deleted = 1 
	...
	set 
		...
		IsActive = case when stg.Deleted = null or stg.Deleted = '' or stg.Deleted = 0 or stg.Deleted = 'False' then 1 else 0 end
	...
	FROM     
	   ...
	   on P.SectionId = alias.ApplAliasID 
       Where alias.ApplAliasID  not in 
	   (Select SectionId from [dbo].[PrecheckFramework_ApplAliasStaging] Where 
	    IsNull(SectionID,'') <> '' and FolderId = @folderId  	
  ...
  BEGIN   
	  ...
	  UPDATE   
	   ...
	   SET   
		...
		SubStatusID = Case When isnull(t.GetNextDate,'') = '' then (Case when a.SubStatusID =29 and a.ApStatus='M' and t.ApStatus = 'M' then 29 else 28 end) else 7 end,
		DOB = t.DOB,  
   ...
   ---------------------------Update primary name ---------------------------
	...
	FROM     
       [dbo].[ApplAlias] alias         
       JOIN   
       [dbo].[PrecheckFramework_ApplStaging] stg   
       ON  
       stg.Apno = alias.Apno   
      where   
       stg.FolderId = @folderId  	
       and stg.CreatedDate >= @DateEntered AND alias.IsPrimaryName = 1
 ...
 IF (charindex('Employment',@sectionList) > 0)  
  ...
  END  
    ...
    BEGIN                                   
		 ...
		 BEGIN 
			  Exec DBO.[CreateCrims_FromStaging] @apno, @FolderId,   @DateEntered
		 END 
