--[dbo].[PrecheckFramework_GetApplicationByFilter] @RecordLimit='1000',@First='Douglas',@ClientOperation='Begins With',@DebugQ=0
...
-- =============================================  
CREATE PROCEDURE [dbo].[PrecheckFramework_GetApplicationByFilter_10292013]  
...
SET @SQL = @SQL + ' IsNull(Investigator,'''') as Investigator,A.ApStatus,A.APNO,cast(A.DOB as varchar) DOB, A.ApDate ,cast(ReopenDate as varchar) as ReOpenDate,null as AvailableDate,a.EnteredBy as EnteredBy, DATEDIFF(day, A.ApDate, getdate()) AS ElapseDays, A.SSN, A.Last,A.First, Replace(C.Name,'''','''''''') as ClientName, A.UserID AS ClientCAM    
