...
-- =============================================      
--[dbo].[PrecheckFramework_GetApplication] 3530509,1,'AllSections','ddegenaro',0  
/*   
...
*/  
CREATE PROCEDURE [dbo].[PrecheckFramework_GetApplication04062017](@apno int = null,@includeHistory bit,@sectionList varchar(100) = null,@username varchar(20) = null,@lockAppl bit = 0)      
AS      
...
select  
	...
	--ad.APNO = cc.APNO 	
	where Replace(ad.SSN,'-','') = Replace(@ssn,'-','') ORDER BY DateCreated desc
	UNION ALL
...
END
 ...
 IF (SELECT COUNT(Apno) from dbo.ApplAlias where Apno = @apno) > 0
     BEGIN
		SELECT DISTINCT 'ApplAlias' as SectionName,ApplAliasID as SectionId,First,Middle,Last,Generation from dbo.ApplAlias 
