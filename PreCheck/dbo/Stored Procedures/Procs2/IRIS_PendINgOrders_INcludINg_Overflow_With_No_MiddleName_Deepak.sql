...
EXEC [IRIS_PendINgOrders_INcludINg_Overflow_With_No_MiddleName_Deepak] 2480

EXEC [IRIS_PendINgOrders] null,1

EXEC [IRIS_PendINgOrders_INcludINg_Overflow_With_No_MiddleName_Deepak] null,1


*/


CREATE procedure [dbo].[IRIS_PendINgOrders_INcludINg_Overflow_With_No_MiddleName_Deepak] (@County INt = null,@CountyList BIT = 0)   
AS  
SET NOCOUNT ON
...
ELSE -- Return the pendINg list per county
	BEGIN
		...
		ORDER BY CAST(ISNULL(Ordered,'1/1/1900') AS DateTime)
			...
			WHERE RowNumber NOT IN (SELECT MAX(t1.RowNumber)
									FROM #tmpSearches AS t1
									INNER JOIN #tmpSearches AS t2 ON t1.apno = t2.APNO AND ((t1.First = t2.Last AND t1.Last = t2.First) or (T1.First = SUBSTRING(T2.First,1,3) AND T1.Last = T2.Last))
									GROUP BY t1.APNO
								   )
			ORDER BY APNo

			SELECT distinct T1.SectionID, T1.Apno, T1.County, T1.Cnty_No, T1.Ordered, T1.Last, T1.First, T1.Middle, T1.DOB, T1.KnownHits
			FROM #tmpTransposeSearch AS T1
			INNER JOIN #tmpTransposeSearch AS T2 ON T1.APNO = T2.APNO AND (T1.First = SUBSTRING(T2.First,1,3) AND T1.Last = T2.Last)
