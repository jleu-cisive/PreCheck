...
-- =============================================
CREATE PROCEDURE [dbo].[Last_Section_Completed_Detail_Amy]
(
...
BEGIN
	...
	--DECLARE 	@StartDate DATETIME='06/01/2018',
	--			@EndDate DATETIME='06/30/2018',
	--			@Clno INT=0,
	--			@AffiliateID INT=29
	...
	/* below can be added/used for future when the table has long time data so that we don't need to scan the whole table. for now it is fine.*/
	 DECLARE	@MinApdate datetime, @MaxApDate datetime ---in order to get fast query, here i only find changeddate in 7 years before for the apdate 
	  SELECT @MinApdate=min(apdate),@MaxApDate=Max(apdate)  FROM #tmp --ORDER BY ApDate, APNO
	  SET @MinApdate = DateAdd(year, -1,@MinApdate)
	  --SET @MaxApDate = DateAdd(year, 1,@MaxApDate)
	  --SELECT @MinApdate, @MaxApDate


  SELECT 'Employmet' AS ComponentType, E.APNO, MAX(C.ChangeDate) OVER (PARTITION BY Apno) AS [DateClosed] INTO #tmpEmployment 
  ...
  AND E.APNO IN (SELECT APNO FROM #tmp) 
  AND C.ChangeDate >= @MinApdate --AND C.ChangeDate <= @MaxApDate
  ORDER BY C.ChangeDate DESC
  --SELECT * FROM #tmpEmployment
  
  SELECT 'Education' AS ComponentType, E.APNO, MAX(C.ChangeDate) OVER (PARTITION BY Apno) AS [DateClosed] INTO #tmpEducation 
  FROM ChangeLog AS C(NOLOCK) 
  INNER JOIN Educat AS E(NOLOCK) ON C.ID = E.Educatid 
  WHERE TableName = 'Educat.SectStat' 
  AND NewValue IN ('2','3','4','5','6','7','A','B') AND E.IsOnReport = 1 AND 
  E.APNO IN (SELECT APNO FROM #tmp)
  AND C.ChangeDate >= @MinApdate --AND C.ChangeDate <= @MaxApDate 
  ORDER BY C.ChangeDate DESC
  --SELECT * FROM #tmpEducation

  SELECT 'Personal Reference' AS ComponentType, P.APNO, MAX(C.ChangeDate) OVER (PARTITION BY Apno) AS [DateClosed] INTO #tmpPersonalReference 
  FROM ChangeLog AS C(NOLOCK) 
  ...
  AND P.APNO IN (SELECT APNO FROM #tmp)
  AND C.ChangeDate >= @MinApdate --AND C.ChangeDate <= @MaxApDate  
  ORDER BY C.ChangeDate DESC
  --SELECT * FROM #tmpPersonalReference

  SELECT 'Crim' AS ComponentType, X.APNO, MAX(C.ChangeDate) OVER (PARTITION BY Apno) AS [DateClosed] INTO #tmpCrim 
  FROM ChangeLog AS C(NOLOCK) 
  INNER JOIN Crim AS X(NOLOCK) ON C.ID = X.CrimID 
  WHERE TableName = 'Crim.Clear' 
  AND NewValue IN ('T','F','P') AND X.IsHidden = 0 
  AND X.APNO IN (SELECT APNO FROM #tmp) 
  AND C.ChangeDate >= @MinApdate --AND C.ChangeDate <= @MaxApDate  
  ORDER BY C.ChangeDate DESC
  --SELECT * FROM #tmpCrim
