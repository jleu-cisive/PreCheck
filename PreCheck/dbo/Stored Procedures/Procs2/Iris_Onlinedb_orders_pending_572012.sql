-- Alter Procedure Iris_Onlinedb_orders_pending_572012
...
CREATE PROCEDURE [dbo].[Iris_Onlinedb_orders_pending_572012] AS
...
AND (dbo.Appl.InUse IS NULL ) AND (Appl.ApStatus = 'p' OR Appl.ApStatus = 'w')
group by dbo.Iris_Researchers.R_Name, dbo.Iris_Researchers.R_Firstname, dbo.Crim.b_rule, dbo.Iris_Researchers.R_Lastname, 
  --ISNULL(dbo.Crim.readytosend, 0) , 
dbo.Crim.readytosend,
dbo.Iris_Researchers.R_id , dbo.Iris_Researchers.R_Delivery, dbo.Crim.CNTY_NO,
dbo.Counties.A_County , 
  dbo.Counties.State, dbo.Crim.IRIS_REC,
dbo.Iris_Researcher_Charges.Researcher_Aliases_count,dbo.Appl.Alias1_Last,
dbo.Appl.Alias1_Middle, dbo.Appl.Alias1_First
,dbo.Appl.Alias2_Last,
dbo.Appl.Alias2_Middle, dbo.Appl.Alias2_First
,dbo.Appl.Alias3_Last,
dbo.Appl.Alias3_Middle, dbo.Appl.Alias3_First
,dbo.Appl.Alias4_Last,
dbo.Appl.Alias4_Middle, dbo.Appl.Alias4_First
)A
group by
A.R_Name,
A.R_Firstname,
...
A.State,
A.IRIS_REC
order by case when sum(A.readytosend) < count(A.readytosend) then 0
else 1 end
...
WHERE    (ir.R_Delivery = 'onlinedb') 
  AND (c.clear = 'R')  
  AND (c.IRIS_REC = 'yes') 
  AND (c.batchnumber IS NULL) 
  AND ((DATEDIFF(mi, c.last_updated, GETDATE()) >= 1)  
  and (ct.cnty_no <> 2480)  or (DATEDIFF(mi, c.last_updated, GETDATE()) >= 20)  
  and (ct.cnty_no = 2480)) 
  AND (a.InUse IS NULL ) 
  AND (c.readytosend=0)
