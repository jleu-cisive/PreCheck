...
-- ==================================================================================
CREATE PROCEDURE [dbo].[GetZipCrimNoneMappingCounties_vtest] 
AS
BEGIN
	...
	CREATE INDEX IX_tmpAllRecords_02 ON #tmpAllRecords(CrimID)
    ...
    UPDATE t 
	SET t.PartnerReferenceLeadNumber = m.ExternalID
	...
	(
		...
		FROM #tmpAllRecords AS t
		INNER JOIN dbo.Crim c on t.APNO = c.APNO AND T.CNTY_NO = C.CNTY_NO
		WHERE T.PartnerReferenceLeadNumber = '0'
	...
	(
		...
		FROM GetOriginalLeadNumber AS d
		INNER JOIN GetDockets AS C ON D.APNO = C.APNO AND D.CNTY_NO = C.CNTY_NO
	)
	...
	INNER JOIN #tmpGetOriginalLeadNumberForEachDevelopedLead t ON d.apno = t.apno AND d.[state] = T.[state] --and d.CrimID = t.CrimID
	WHERE d.PartnerReferenceLeadNumber = '0'
	 ...
	 (
		...
		FROM GetInActiveLeadsFromMapping   AS g With (nolock)
		INNER JOIN dbo.Crim c With (nolock) ON g.CNTY_NO = C.CNTY_NO AND g.APNO = c.APNO
		WHERE C.IsHidden = 0
	 ) 
	 ...
	 FROM GetActiveLeads as g with (nolock) 
	 INNER JOIN dbo.PreCheckZipCrimComponentMap  AS M with (nolock) ON g.CrimID = m.SectionUniqueID
	 WHERE g.RowNumber = 1
	   AND g.CrimCrimID NOT IN (SELECT P.SectionUniqueID FROM dbo.PreCheckZipCrimComponentMap AS P  )

	-- Update Crim's PartnerReferenceLeadNumber with derived Lead Number
	--SELECT 'Update Crim' AS TableName, c.APNO, C.CrimID, t.CrimID, c.PartnerReferenceLeadNumber, t.PartnerReferenceLeadNumber
		UPDATE C SET C.PartnerReferenceLeadNumber = t.PartnerReferenceLeadNumber
	FROM #tmpAllRecords t
