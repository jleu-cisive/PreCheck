...
--[dbo].[Overseas_Education_by_Clients_by_Date_Range] '03/01/2021', '03/31/2021', '0', 230    
...
-- Modified By Arindam Mitra on 08/23/2023 for Ticket No.106972,remove duplicate records.

-- =============================================    
CREATE PROCEDURE [dbo].[Overseas_Education_by_Clients_by_Date_Range]    
...
--   @AffiliateId int = 4  
 ...
 IF OBJECT_ID('tempdb..#Facility') IS NOT NULL DROP TABLE #Facility  -- Code added against HDT#106972 
...
--LEFT JOIN HEVN.dbo.Facility F (NOLOCK) ON isnull(A.DeptCode,0) = F.FacilityNum  --Humera Ahmed on 3/2/2020 for HDT#67981  --Code commented against HDT#106972
LEFT JOIN #Facility F with(NOLOCK) ON isnull(A.DeptCode,0) = F.FacilityNum AND (c.WebOrderParentCLNO = F.ParentEmployerID OR C.WebOrderParentCLNO = F.EmployerID) --Code added against HDT#106972  
Left join dbo.SectSubStatus sss (nolock) on E.SectStat = sss.SectStatusCode and E.SectSubStatusID = sss.SectSubStatusID  
WHERE   
A.OrigCompDate >= @StartDate  --Humera Ahmed on 3/2/2020 for HDT#67981  
