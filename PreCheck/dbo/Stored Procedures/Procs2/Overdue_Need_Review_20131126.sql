...
Create PROCEDURE [dbo].[Overdue_Need_Review_20131126] AS
...
SELECT A.Apno, A.ApStatus, A.UserID, A.Investigator,PC_Time_Stamp,
	...
	WHERE (Crim.Apno = A.Apno)
	  AND (Crim.Clear IS NULL) 	  AND (Crim.IsHidden = 0))
        AS Crim_Count,
       (SELECT 0)
        AS Civil_Count,
	...
	WHERE (Educat.Apno = A.Apno)
	  AND ( Educat.SectStat = '0') AND Educat.IsOnReport = 1) AS Educat_Count,
       (SELECT COUNT(*) FROM ProfLic with (nolock) 
	WHERE (ProfLic.Apno = A.Apno)
	  AND ( ProfLic.SectStat = '0') AND ProfLic.IsOnReport = 1) AS ProfLic_Count,
       (SELECT COUNT(*) FROM PersRef with (nolock) 
	WHERE (PersRef.Apno = A.Apno)
	  AND ( PersRef.SectStat = '0') AND PersRef.IsOnReport = 1) AS PersRef_Count,
...
where Crim_Count = 0 and Civil_Count = 0 and Credit_Count = 0 and DL_Count = 0 and Empl_Count = 0 and Educat_Count = 0 and ProfLic_Count = 0 and PersRef_Count = 0 and Medinteg_Count = 0
