-- Alter Procedure Iris_Pending_DPS_InHouse
CREATE PROCEDURE [dbo].[Iris_Pending_DPS_InHouse] AS
...
WHERE    (dbo.Appl.ApStatus = 'p' OR  dbo.Appl.ApStatus = 'w') and (dbo.Crim.IRIS_REC = 'yes') AND (dbo.Crim.Clear = 'O') and
 (NOT (dbo.Crim.batchnumber IS NULL)) AND (dbo.Iris_Researchers.R_Delivery = 'InHouse') AND  (dbo.Iris_Researchers.R_id = 262)
