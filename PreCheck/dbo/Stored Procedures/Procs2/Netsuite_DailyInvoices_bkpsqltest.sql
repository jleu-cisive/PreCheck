...
Create PROCEDURE  [dbo].[Netsuite_DailyInvoices_bkpsqltest]
...
i.Invoicenumber [Memo], InvoiceDate TranDate,
-- CONVERT(CHAR(4), InvoiceDate, 100) + CONVERT(CHAR(4), i.InvoiceDate, 120) as PostingPeriod
'Jan 2025' as PostingPeriod
...
from [hou-sqltest-01].Precheck.[dbo].[NetSuiteDailyInvoice]  i 
inner join [hou-sqltest-01].[Precheck_PreProd].[dbo].appl a on i.Invoicenumber = a.APNO
...
from [hou-sqltest-01].Precheck.[dbo].[NetSuiteDailyInvoice] where leadtypeid is not null   group by InvoiceNumber,leadtypeid
--union all
...
where i.Invoicenumber in (5440189,5440195,5440194,5440193,5440191,5440190)
-- (5440117,5440118,5440119,5440120,5440121)
...
Union All
select t.InvoiceNumber  as InvoiceId, 'HC-'+ cast(v.Employerid as varchar(10)) as CustomerId, Transactionid [Memo], Transdate TranDate,-- CONVERT(CHAR(4), Transdate, 100) + CONVERT(CHAR(4), Transdate, 120) as PostingPeriod
'Jan 2025' as PostingPeriod
...
from [hou-sqltest-01].hevn.[dbo].[CCTransactionLog] t inner join [hou-sqltest-01].hevn.[dbo].Verify v on t.VerifyID = v.VerifyID 
inner join [hou-sqltest-01].hevn.[dbo].Verifier ver on v.VerifierID = ver.VerifierID
where transdate >  '8/12/2021' and transdate <  '8/13/2021' and T.verifyid is not null 
and t.verifyid in (586956,586955,586954,586953,586952,586951)
) z
...
from [hou-sqltest-01].[PrecheckServices].[dbo].[Payment] i
inner join   [hou-sqltest-01].[PrecheckServices].[dbo].ccpayment cc on i.ccpaymentid = cc.ccpaymentid
inner join appl a on i.AppNo = a.APNO
inner join client c on a.clno = c.CLNO
where APPNO in   (5440117,5440118,5440119,5440120,5440121)
--(5888700,5888750,5888753) 
...
union all
select Transactionid PaymentId,'HC-'+ cast(Employerid as varchar(10))   AccountId, t.InvoiceNumber  as InvoiceId, TransDate 	InvoiceDate, TransDate	PaymentDate, Baseamount Amount, BilledTax	Tax, TransREF	PNRefNo, CCType	PaymentMethod 
from [hou-sqltest-01].hevn.[dbo].[CCTransactionLog] t inner join [hou-sqltest-01].hevn.[dbo].Verify v on t.VerifyID = v.VerifyID 
where transdate >  '8/12/2021' and transdate <  '8/13/2021' and T.verifyid is not null 
and t.verifyid in (586956,586955,586954,586953,586952,586951)
) t
