-- Alter Procedure Iris_CriminalWorkSheetPerVendor
...
Execution : 
	...
	Modified by Lalit to optimize it on 19 sep 2022
	Modified by Lalit to optimize it on 31 oct 2022 for #59433
*/


CREATE PROCEDURE [dbo].[Iris_CriminalWorkSheetPerVendor]
@VendorID int
AS

DROP TABLE IF EXISTS #tempcrimid
SELECT CrimID AS SectionKeyID INTO #tempcrimid FROM Crim WITH (NOLOCK) WHERE VendorID = @Vendorid AND Clear in( 'O','W')
...
SELECT CrimID, APNO, County,clear,  Ordered, Name, DOB,  SSN, CaseNo, Date_Filed,  Degree,Offense,  Disposition,  Sentence, 
       ...
       (      
              (Select C.CrimID as CrimID, 
                           ...
                           CONCAT('Client ID ',cl.clno,', Affiliate ID ',ISNULL(cl.affiliateid,'0'),',',ISNULL(C.CRIM_SpecialInstr, '')) as CRIM_SpecialInstr, 
                     ...
                     LEFT JOIN (SELECT t.SectionKeyID
                                         ,STUFF((SELECT DISTINCT ' / ' + isnull(A.Last, '') + ', ' +  isnull(A.First, '') + ' ' + isnull(A.Middle, '')
                                                       FROM ApplAlias AS A
                                                       INNER JOIN dbo.ApplAlias_Sections AS S ON A.ApplAliasId = S.ApplAliasId
