...
CREATE PROCEDURE [dbo].[PBI_CSH_Executive_Dashboard]
...
-----------AdverseAction...... 8 Sec for 4662 records---------------------------------


	if @TableName='AdverseAction'
		BEGIN
			...
			(
				...
				WHERE c.weborderparentclno = 13966 and Year(a.OrigCompDate) >= 2019
			), 
			cteReportList AS
			(
				...
				WHERE c.weborderparentclno = 13966
...
-----------ApplicantContacts......  2:40 mint for 47394 records---------------------------------


	if @TableName='ApplicantContacts'
		BEGIN
	...
	(
		...
		WHERE year(a.OrigCompDate) >= 2019
		--AND month(a.apDate) = 1
		--AND c.AffiliateID IN (147,149)
		AND c.weborderparentclno = 13966
...
-----------Applications......  19 Sec for 86781 records---------------------------------


	if @TableName='Applications'
		BEGIN

			SELECT GETDATE() DateDataRefreshed, c.AffiliateID, a.APNO, a.Last + ', ' + a.First FullName, a.Sex, a.ApStatus AppStatus, a.EnteredVia
	...
	INNER JOIN refAffiliate aff (nolock)  --Each client has an affiliate
		...
		LEFT JOIN (SELECT FacilityID, FacilityNum, FacilityName, Division, IsOneHR, ROW_NUMBER() OVER (PARTITION by FacilityNum ORDER BY FacilityID DESC) RowNumber from HEVN.dbo.Facility WHERE ParentEmployerID = 7519) f ON isnull(a.deptcode,0) = f.FacilityNum AND f.RowNumber=1
	WHERE year(a.OrigCompDate) >= 2019
	--AND c.AffiliateID IN (147,149)
	AND c.weborderparentclno = 13966
	AND a.OrigCompDate IS NOT NULL
...
-----------Billing...... 27 Sec for 1241858 records---------------------------------
...
-----------Criminal......3:58 mint for 475965 records ---------------------------------
...
-----------Education......1:7 mint for 79655 records ---------------------------------
...
-----------Employment......24 sec for 109330 records ---------------------------------
...
-----------FeelsLikeTAT......1:11 mint for 107008 records---------------------------------


	if @TableName='FeelsLikeTAT'
		BEGIN
			...
			SELECT distinct a.APNO, vc.ClientId AS [Client Number], vc.ClientName AS [Client Name],RA.Affiliate AS [Affiliate Name],
				   ...
				   WHERE year(OrigCompDate) >= 2019
				   --AND vc.AffiliateID IN (147,149)
				   AND vc.parentid = 13966
				   AND OrigCompDate IS NOT NULL

		END


-----------License...... 26 sec for 36702 records---------------------------------
	...
	if @TableName='License'
		BEGIN
			...
			(
				...
				WHERE cl.NewValue IN ('1','2','3','4','5','6','7','8','A','E','B') AND cl.TableName = 'ProfLic.SectStat' AND YEAR(ChangeDate) >= 2019
				GROUP BY cl.ID
			), cteLicense AS
			(
...
-----------PersonalReferences...... 18 sec for 9632 records---------------------------------


	if @TableName='PersonalReferences'
		BEGIN
			...
			(
				...
				WHERE cl.NewValue IN ('1','2','3','4','5','6','7','8','A','E','B') AND cl.TableName = 'PersRef.SectStat' AND YEAR(ChangeDate) >= 2019
				GROUP BY cl.ID
			), ctePRef AS
			(
				...
				, CASE WHEN cl.ChangeDate IS NOT NULL THEN cl.ChangeDate
						...
						END ComponentClosingDate
				FROM cteRefIds pl
				INNER JOIN dbo.SectStat PRefStatus (NOLOCK)
...
-----------SanctionCheck...... 4 sec for 105461 records---------------------------------


	if @TableName='SanctionCheck'
		BEGIN
