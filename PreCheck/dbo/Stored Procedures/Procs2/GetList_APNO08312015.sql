...
CREATE PROCEDURE [dbo].[GetList_APNO08312015]
(
	@IsCAM bit = 0
...
BEGIN
	...
	SET @SQL = @SQL + ' A.Investigator,A.ApStatus,A.APNO, A.ApDate, A.ReopenDate, DATEDIFF(day, A.ApDate, getdate()) AS ElapseDays, A.SSN, A.Last, A.First, C.Name AS ClientName, A.UserID AS ClientCAM,isnull(AAD2.Crim_SelfDisclosed,isnull(AAD.Crim_SelfDisclosed,0)) Crim_SelfDisclosed
...
BEGIN
	SELECT distinct A.Investigator,A.ApStatus,A.APNO, A.ApDate, A.ReopenDate, DATEDIFF(day, A.ApDate, getdate()) AS ElapseDays, A.SSN, A.Last, A.First, C.Name AS ClientName, A.UserID AS ClientCAM,isnull(AAD2.Crim_SelfDisclosed,isnull(AAD.Crim_SelfDisclosed,0)) Crim_SelfDisclosed,GetNextDate
	FROM dbo.Appl A (NOLOCK) 
		LEFT OUTER JOIN dbo.ApplAdditionalData AAD with (Nolock) ON ( (A.CLNO = AAD.CLNO AND  A.SSN = AAD.SSN AND AAD.SSN IS NOT NULL ))
		LEFT OUTER JOIN dbo.ApplAdditionalData AAD2 with (Nolock) ON (A.APNO = AAD2.APNO AND  AAD2.APNO IS NOT NULL)
		INNER JOIN dbo.Client C (NOLOCK) ON A.CLNO = C.CLNO
...
--schapyala created a temp table to prevent the repeated queries below - 07/15/14
		...
		isnull(CCredit.cnt,0)  CCreditPendingCount,isnull(DL.cnt,0)  DLPendingCount
		into #tmpAppl
		...
		INCLUDE ([Investigator],[APNO],[ApDate],[ReopenDate],[ElapseDays],[SSN],[Last],[First],[ClientName],[ClientCAM],[SentPending],[Crim_SelfDisclosed],[emplPendingCount],[EducatPendingCount],[PersRefPendingCount],[CCreditPendingCount],[DLPendingCount])
...
--				AND SS.MainStatusID = 3	--completed
	...
	BEGIN
		...
		UNION ALL
		Select distinct A.Investigator,A.ApStatus,A.APNO, A.ApDate, A.ReopenDate, A.ElapseDays, A.SSN, A.Last, A.First, A.ClientName, A.ClientCAM,
				1 as Available, A.SentPending,A.Crim_SelfDisclosed
		From #tmpAppl A
		Where A.ApStatus = 'P'
...
--		ORDER BY available desc,A.apno
	...
	BEGIN
        SELECT distinct A.Investigator,A.ApStatus,A.APNO, A.ApDate, A.ReopenDate, DATEDIFF(day, A.ApDate, getdate()) AS ElapseDays, A.SSN, A.Last, A.First, C.Name AS ClientName, A.UserID AS ClientCAM,isnull(AAD2.Crim_SelfDisclosed,isnull(AAD.Crim_SelfDisclosed,0)) Crim_SelfDisclosed
		FROM dbo.Appl A (NOLOCK) 
			INNER JOIN dbo.Client C (NOLOCK) ON A.CLNO = C.CLNO
			LEFT OUTER JOIN dbo.ApplAdditionalData AAD with (Nolock) ON ( (A.CLNO = AAD.CLNO AND  A.SSN = AAD.SSN AND AAD.SSN IS NOT NULL ))
			LEFT OUTER JOIN dbo.ApplAdditionalData AAD2 with (Nolock) ON (A.APNO = AAD2.APNO AND  AAD2.APNO IS NOT NULL)
