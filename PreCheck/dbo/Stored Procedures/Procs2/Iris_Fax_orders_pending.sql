...
CREATE PROCEDURE [dbo].[Iris_Fax_orders_pending]  AS
...
--isnull(datediff(mi,getdate(),dbo.LookupTimeZoneByState (Iris_Researchers.R_City,Iris_Researchers.R_State_Province,getdate(),iris_researchers.cutoff)),999999999) as mycutoff,
       ...
       Crim.CNTY_NO,
                          isnull(Crimenteredtime,(SELECT     MIN(z.crimenteredtime)
                            FROM          Crim z WITH (NOLOCK) 
                            WHERE      (z.cnty_no = crim.cnty_no) AND (z.clear = 'R') AND (z.iris_rec = 'yes'))) AS crim_time,
							Counties.A_County AS county, 
...
--(
---- If there is an stuck in the que check for application it may be in Final status
-- SELECT  distinct Iris_Researchers.R_Name, 
--         Iris_Researchers.R_Firstname, 
--         Crim.b_rule, 
--         Iris_Researchers.R_Lastname,
--         Iris_Researchers.R_State_Province,
--         Case When 
...
--            AND (z.clear = 'R') AND (z.iris_rec = 'yes')) AS crim_time,
--         Counties.A_County AS county, 
--         Counties.State, 
--         Crim.IRIS_REC
--FROM     Crim WITH (NOLOCK) 
--INNER JOIN  Counties WITH (NOLOCK) ON dbo.Crim.CNTY_NO = Counties.CNTY_NO 
--INNER JOIN  Appl WITH (NOLOCK) ON dbo.Crim.APNO =  Appl.APNO 
--LEFT OUTER JOIN Iris_Researchers WITH (NOLOCK) ON dbo.Crim.vendorid = Iris_Researchers.R_id
...
--WHERE     (dbo.Iris_Researchers.R_Delivery = 'fax') 
--      AND (Crim.clear = 'R') 
...
--                  dbo.Counties ct WITH (NOLOCK) ON c.CNTY_NO = ct.CNTY_NO INNER JOIN
