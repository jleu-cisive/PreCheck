CREATE PROCEDURE [dbo].[iris_ws_update_crim_private_notes]
    @crim_id BIGINT,
...
AS
DECLARE @CRLF CHAR(2);
DECLARE @Priv_Notes VARCHAR(max);
...
BEGIN
  ...
  /*  SET @temp_degree = CASE
    ...
    select @Priv_Notes = Priv_Notes from dbo.crim where crimid = @crim_id;
    if (@Priv_Notes <>'')
     begin
      set @Priv_Notes = 'Additional Items - ' + lower(@Priv_Notes);
     end

    UPDATE dbo.crim
      SET Priv_Notes = 'Order Status: ' + @temp_order_status
        + @CRLF
