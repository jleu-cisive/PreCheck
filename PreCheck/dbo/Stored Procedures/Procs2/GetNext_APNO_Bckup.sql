...
CREATE PROCEDURE [dbo].[GetNext_APNO_Bckup]
(
	...
	BEGIN
		...
		FROM dbo.Appl A
			INNER JOIN dbo.Client C ON A.CLNO = C.CLNO
			 ...
			 LEFT JOIN (SELECT COUNT(1) cnt,APNO FROM dbo.Crim	   WHERE ISNULL(Clear, '') IN ('','R','M','O', 'V','W','X','E','N','I','Q') Group by Apno) Crim on A.APNO = Crim.APNO
...
BEGIN
	...
	FROM dbo.Appl A
		INNER JOIN dbo.SubStatus SS ON ISNULL(A.SubStatusID, 2) = SS.SubStatusID
			...
			AND (A.NeedsReview LIKE '%2' or A.NeedsReview LIKE '%5' or A.NeedsReview LIKE '%3')
			AND A.ApStatus = 'P'
	...
	BEGIN
		...
		FROM dbo.Appl A
			INNER JOIN dbo.Client C ON A.CLNO = C.CLNO
				...
				AND ISNULL(A.InUse, '') = ''
				AND (A.NeedsReview LIKE '%2' or A.NeedsReview LIKE '%5' or A.NeedsReview LIKE '%3')
			LEFT OUTER JOIN dbo.ClientWeight W ON A.CLNO = W.CLNO
		WHERE (C.Investigator1 = @Investigator OR C.Investigator2 = @Investigator)
...
BEGIN
	IF @IsCAM = 0
		...
		INSERT INTO [PreCheck].[dbo].[ApplGetNextLog]
           ([APNO]
           ,[username]
           ,[QueueType]
           ,[CreatedDate])
...
--USE [PreCheck]
--GO
--/****** Object:  StoredProcedure [dbo].[GetNext_APNO]    Script Date: 09/18/2013 09:27:04 ******/
--SET ANSI_NULLS ON
--GO
--SET QUOTED_IDENTIFIER ON
--GO

--ALTER PROCEDURE [dbo].[GetNext_APNO]
