CREATE PROCEDURE [dbo].[PBI_UTSW_Executive_Access_Dashboard]  
	...
	if @TableName='AdverseAction'  
	  BEGIN  
			...
			(
				...
				WHERE c.affiliateid = 260 and Year(a.OrigCompDate) >= 2020
			), 
			cteReportList AS
			(
	  ...
	  BEGIN 
			...
			(
				...
				INNER JOIN cteCrimIds cr
					ON cl.ID = cr.CrimID
				WHERE cl.NewValue IN ('F','T','P') AND YEAR(ChangeDate) >= 2020
				GROUP BY cl.ID
			), cteCrims AS
			(
	...
	if @TableName='Education'  
	  BEGIN  
			...
			(
				...
				WHERE cl.NewValue IN ('1','2','3','4','5','6','7','8','A','E','B') AND cl.TableName = 'Educat.SectStat' AND YEAR(ChangeDate) >= 2020
				GROUP BY cl.ID
			), cteEdu AS
			(
	...
	if @TableName='Employment'  
	  BEGIN 
			...
			(
				...
				WHERE cl.NewValue IN ('1','2','3','4','5','6','7','8','A','E','B') AND cl.TableName = 'Empl.SectStat' AND YEAR(ChangeDate) >= 2020
				GROUP BY cl.ID
			), cteEmpl AS
			(
	...
	if @TableName='License'  
	  BEGIN  
			...
			(
				...
				WHERE cl.NewValue IN ('1','2','3','4','5','6','7','8','A','E','B') AND cl.TableName = 'ProfLic.SectStat' AND YEAR(ChangeDate) >= 2020
				GROUP BY cl.ID
			), cteLicense AS
			(
	...
	if @TableName='PersonalReferences'  
	  BEGIN
			...
			(
				...
				WHERE cl.NewValue IN ('1','2','3','4','5','6','7','8','A','E','B') AND cl.TableName = 'PersRef.SectStat' AND YEAR(ChangeDate) >= 2020
				GROUP BY cl.ID
			), ctePRef AS
			(
