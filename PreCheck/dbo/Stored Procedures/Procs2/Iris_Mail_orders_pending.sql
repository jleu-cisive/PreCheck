-- Alter Procedure Iris_Mail_orders_pending




CREATE PROCEDURE [dbo].[Iris_Mail_orders_pending] AS
...
--WHERE     (dbo.Iris_Researchers.R_Delivery = 'mail') AND (dbo.Crim.Clear IS NULL or Crim.clear = 'R')  AND (dbo.Crim.IRIS_REC = 'yes') AND (dbo.Crim.batchnumber IS NULL) AND 
...
SELECT   distinct dbo.Iris_Researchers.R_Name, 
         ...
         dbo.Crim.CNTY_NO,
		 isnull(Crim.Crimenteredtime,(SELECT     MIN(z.crimenteredtime)
                           FROM Crim z
...
WHERE (dbo.Iris_Researchers.R_Delivery = 'mail') 
