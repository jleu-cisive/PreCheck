-- Alter Procedure Iris_Pending_Fax
CREATE PROCEDURE [dbo].[Iris_Pending_Fax] AS
...
WHERE     (dbo.Crim.IRIS_REC = 'yes') AND (dbo.Crim.Clear = 'O') and
(dbo.Appl.ApStatus = 'p' OR dbo.Appl.ApStatus = 'w') AND (NOT (dbo.Crim.batchnumber IS NULL)) AND (dbo.Iris_Researchers.R_Delivery = 'fax')
