...
--[dbo].[IRIS_PendingOrders] 56,0
--[IRIS_PendingOrders_Baxter] null,1,1

--[IRIS_PendingOrders_Baxter] null,1

CREATE procedure [dbo].[IRIS_PendingOrders_Baxter] (@County int = null,@CountyList BIT = 0,@ForIntegration BIT = 0)   
as  
SET NOCOUNT ON
IF @CountyList = 1 -- Return a distinct list of counties
	BEGIN
		...
		select * from #tmpCountyList order by Cnty_No
...
ELSE -- Return the pending list per county
	BEGIN
		...
		IF (@ForIntegration = 1)
				...
				Select Distinct top 5 'Crim' Section,
					SectionID,
					Apno,
					County,
					Cnty_No,
