...
-- =============================================
CREATE PROCEDURE [dbo].[LicensesNotProcessedToAIMS]
...
--CREATE TABLE #TABLE1  ( LICENSEID INT )
              ...
              Begin
			   ...
			   SELECT @xmlDoc=REPLACE(REPLACE(REPLACE(REPLACE(Request,' – ',''),'’',''),'"',' '),'©',' ')--REPLACE([Response],'©',' ')
					FROM         DataXtract_Logging 
					WHERE DataXtract_Loggingid  = @DataXtract_LoggingId
			  --select @DataXtract_LoggingId, @xmlDoc
                    ...
                    INSERT INTO dbo.TCHAppno ( Apno)
					 SELECT 
                           *  
                     FROM OPENXML (@xmlHandle, '//Item', 2) WITH
                           (
                                  Licenseid INT 'Licenseid'
                                  --ProvidedFirst  VARCHAR(100) 'ProvidedFirst',
                                  --  ProvidedLast VARCHAR(100) 'ProvidedLast',
                                  --ProvidedLicenseNumber VARCHAR(20) 'ProvidedLicenseNumber',
                                  --ProvidedExpirationDate VARCHAR(20) 'ProvidedExpirationDate',
                                  --NotFound VARCHAR(20) 'NotFound',

                                  --ExpirationDate  VARCHAR(20) 'ExpirationDate',
                                  --IssueDate VARCHAR(20) 'IssueDate',
                                  --LicenseNumber VARCHAR(20) 'LicenseNumber',
                                  --LicenseType VARCHAR(100) 'LicenseType',
                                  --State VARCHAR(20) 'State',
                                  --Name Varchar(200) 'Name',
                                  --Last VARCHAR(100) 'Last',

                                  --First VARCHAR(100) 'First',
                                  --LicenseStatus VARCHAR(100) 'LicenseStatus',
                                  --DisciplinaryAction VARCHAR(max) 'DisciplinaryAction'
                                  --,AdditionalType VARCHAR(300) 'AdditionalType',
                                  --MultiStateStatus VARCHAR(100) 'MultiStateStatus',
                                  --ResultStatus VARCHAR(300) 'ResultStatus'
                    ...
                    -- order by ProvidedFirst
					 ...
					 select @counters,@DataXtract_LoggingId
