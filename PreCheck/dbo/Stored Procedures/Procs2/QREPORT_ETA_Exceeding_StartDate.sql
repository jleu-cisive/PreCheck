...
---Testing
/*
EXEC [dbo].[QREPORT_ETA_Exceeding_StartDate] '3115',0 
EXEC [dbo].[QREPORT_ETA_Exceeding_StartDate] '3115','4:177' 
*/
-- =============================================================================  
CREATE PROCEDURE [dbo].[QREPORT_ETA_Exceeding_StartDate]  
@CLNO  INT,  
@AffiliateID Varchar(Max)									-- Added on the behalf for HDT #55504
 --@AffiliateID int											-- Comnt for HDT #55504	  
       ...
       BEGIN  
              ...
              WHERE A.ApStatus NOT IN ('F','M')  
                ...
                AND (A.Clno IN (SELECT CLNO FROM Client (NOLOCK) WHERE CLNO =@clno OR WebOrderParentCLNO = @clno)  
                                 OR (@AffiliateID IS NULL OR C.AffiliateID in (SELECT value FROM fn_Split(@AffiliateID,':'))) -- Added on the behalf for HDT #55504
									--or c.AffiliateID =IIF(@AffiliateID=0,C.AffiliateID,@AffiliateID)						-- Comnt for HDT #55504	 
								 )  
    AND A.StartDate < ETADate  
