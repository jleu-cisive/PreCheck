...
-- =============================================
CREATE PROCEDURE [dbo].[Integration_ClientAdjResultBatch]
...
SET ARITHABORT ON;
--
----This is used only to send for APPS with a Cutoff date of 03/01/2013
...
where a.apstatus = 'F' and a.clno = @CLNO and a.ApDate < @CutoffDate and lastsyncutc is null and ISNULL(a.clientapno,'') <> '' 
and a.compdate>'03/01/2013' 

--SELECT a.APNO FROM appl a (NOLOCK) where a.apstatus = 'F' and a.clno = @CLNO and a.ApDate < @CutoffDate and
--ISNULL(clientapno,'') <> '' 
