...
CREATE proc [dbo].[GetWorkNumberRecords] 
 as
...
--select 0 APNO, 0 CLNO, 0 VendorAccountId, 'P' ApStatus, '' SSN, '' Priv_Notes, 
--'' Pub_Notes , '' [First] , '' [Last] , 0 EmplID , 0 EmplVerifyID , 
--'' Employer , '' Location , '' [state] , '' city , 0 zipcode , 
--'' Position_A , '' Position_V , '' From_A , 0 IsFrom_AYear , '' From_V , '' To_A , 0 IsTo_AYear ,
--'' To_V , '' RFL , '' Salary_V , 0 SectStat , '' Title , 
--'' ver_by , '' web_status , '' name , 0 CodeCount , 0 NoPassThroughCharges , 0 ApprovedPassThroughCharges , '' VerifyType, 0 Isbilled



DECLARE @WorkNumber TABLE  (APNO int,CLNO int,VendorAccountId int, ApStatus char(1), SSN varchar(11), Priv_Notes nvarchar(4000), 
...
ver_by varchar(50), web_status int, name varchar(100), CodeCount int, NoPassThroughCharges  bit, ApprovedPassThroughCharges bit, VerifyType varchar(20), investigator varchar(8))

Insert into @WorkNumber(APNO,CLNO,VendorAccountId, ApStatus, SSN, Priv_Notes, 
						...
						ver_by, web_status, name, CodeCount, NoPassThroughCharges, ApprovedPassThroughCharges, VerifyType, investigator)
SELECT top 50 a.APNO, 
		 A.CLNO,
		 VendorAccountId = (Select top 1 VendorAccountId from dbo.VendorAccounts where VendorAccountName='WorkNumber' and IsActive=1),
		 a.ApStatus, 
		 a.SSN, 
		 '' as Priv_Notes, 
		 '' as Pub_Notes, 
		 ...
		 case when (isnumeric(RTRIM(LTRIM(VerificationSourceCode))) = 1) then RTRIM(LTRIM(i.VerificationSourceCode)) else 0 end as EmplVerifyID, 
		 ...
		 e.Position_A, 
		 e.Position_V,	 
		case when Lower(e.From_A) = 'current' or  Lower(e.From_A) = 'present' or Lower(e.From_A) = 'n/a' then convert(varchar(10),getdate(),101) else e.From_A end as From_A, 
		0, 
		e.From_V, 
		case when Lower(e.To_A) = 'current' or  Lower(e.To_A) = 'present' or Lower(e.To_A) = 'n/a' then convert(varchar(10),getdate(),101) else e.To_A end as To_A, 
		0,
		case when Lower(e.To_V) = 'current' or  Lower(e.To_V) = 'present' then convert(varchar(10),getdate(),101) else e.To_V end as To_V, 
		--e.To_V,
...
--inner join dbo.Integration_Verification_Transaction ivt (NOLOCK) ON e.APNO = ivt.apno
...
--and a.APNO in (5277026)

and (isnull(e.Inuse,'') = '') 
--and (isnull(i.VerificationSourceCode,'') <> '') --REMOVED FOR TALX WATERFALL
and IsNull(a.SSN,'') <> ''
--and isnumeric(i.VerificationSourceCode) = 1 --REMOVED FOR TALX WATERFALL
--and ivt.IsInternalVerification =1
--update dbo.Empl set InUse=null where InUse='WNWinSrv' and web_status=69
...
UPDATE i
		...
		--and i.VerificationSourceCode = cast(w.EmplVerifyID as varchar(10)) --REMOVED FOR TALX WATERFALL
		where w.web_status = 69

--Added by Jenitta for HDT 124312
UPDATE e
SET e.web_status = 0,
	e.SectStat = 9,
	e.SectSubStatusID = null,
	e.InUse=null,
	e.Investigator = 'SJV'
FROM @WorkNumber w 
INNER JOIN dbo.Empl e ON w.EmplID = e.EmplID
WHERE w.ApprovedPassThroughCharges = 0    AND w.web_status = 69           

SELECT distinct w.APNO,w.CLNO, w.VendorAccountId,
w.ApStatus, w.SSN, w.Priv_Notes, w.Pub_Notes, w.[First], w.[Last], w.EmplID, w.EmplVerifyID, 
	       ...
	       case when i.Description like '%Work Number%' then 1 else 1 end as Isbilled,
		   w.investigator
	       FROM @WorkNumber w 
	       left join dbo.invdetail (nolock) i on w.apno = i.apno and cast(w.EmplVerifyID as varchar(10)) = i.subkeychar where web_status = 69 
		   and w.ApprovedPassThroughCharges=1 --Added by Jenitta for HDT 124312
