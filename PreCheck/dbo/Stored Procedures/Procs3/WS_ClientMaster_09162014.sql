...
--WS_ClientMaster 1917,@IncludeClientContacts = 1
...
-- =============================================   
 ...
 --[dbo].[WS_ClientMaster] 5023,@IncludeClientRequirements = 1
CREATE PROCEDURE [dbo].[WS_ClientMaster_09162014]    
 ...
 IF @IncludeClientRequirements = 1    
  ...
  BEGIN    
   ...
   SELECT 'ClientRequirements' Node,C.CLNO, Social PositiveID, [Medicaid/Medicare] SanctionCheck, MVRService,  MVR, PersonalRefNotes,       
       ...
       Req.ProfRef, Req.DOT, Req.SpecialReg, Req.Civil, Req.Federal, COALESCE(Req.Statewide,SW.Description) as Statewide
 ...
 --   --SELECT CLNO, NoteID, NoteType, NoteBy, NoteDate, NoteText FROM dbo.ClientNotes WHERE (CLNO = @CLNO) order by NoteDate desc  
	--SELECT CLNO, NoteID, NoteType, NoteBy, NoteDate, NoteText FROM dbo.ClientNotes WHERE (CLNO = @CLNO) order by LTRIM(cast(NoteText as varchar)) ASC
	  ...
	  (
		  SELECT CLNO, NoteID, 
		  --NoteType,
		  case when (NoteType in ('','CS') OR NoteType IS NULL) THEN 'All' ELSE NoteType END NoteType, --modified by santosh/Doug to include CS, NULL and empty types into ALL client notes - 06/24/2013
