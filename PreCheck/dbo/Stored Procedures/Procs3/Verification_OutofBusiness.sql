...
-- Modified By: Amy Liu on 05/15/2020 for project: IntranetModule-Status-SubStatus 
-- Description: Set SectSubStatusID in Empl table.
-- exec [dbo].[Verification_OutofBusiness] 4281234,'9',74,'Unverified: No Response from Source',null
-- =============================================

CREATE PROCEDURE [dbo].[Verification_OutofBusiness]
	@emplid int,
	...
	BEGIN
		BEGIN TRANSACTION
			...
			BEGIN
				...
				IF(@ExistingPublicNotes IS NULL) BEGIN SET @ExistingPublicNotes=' ' END

				IF @CLNO>0 and EXISTS(SELECT top 1 1 from [dbo].[ClientOutOfBusiness] WHERE  CLNO=@CLNO and [IsActive]=1)
				BEGIN

					--Update Public Note
					DECLARE @NewPublicNotes NVARCHAR(MAX)=NULL;
					SELECT top 1 @NewPublicNotes=[Text] FROM [dbo].[VerificationsNotes] WHERE [VerificationsNotesId]=@publicNotesId;
					IF(@NewPublicNotes IS NOT NULL AND @NewPublicNotes<>'')
						BEGIN
							SET @ExistingPublicNotes=@NewPublicNotes+' '+@ExistingPublicNotes;
						END
				...
				SET @SectStatus='U';--Default if client not exist  ---Changed to be 'U' for project Status-Substatus--AmyLiu on 06/01/2020
				SET @WebStatus=0;--Default if client not exist

				if exists( select * from SectSubStatus sss where sss.ApplSectionID=1 and sss.ResultFound =@resultFound and sss.SectStatusCode = @SectStatus)	
					select  @SectSubStatusID = sss.SectSubStatusID from SectSubStatus sss where sss.ApplSectionID=1 and sss.ResultFound =@resultFound and sss.SectStatusCode = @SectStatus
				else 
					select @SectSubStatusID = sss.SectSubStatusID from SectSubStatus sss where sss.ApplSectionID=1 and sss.SectStatusCode = 'U' and sss.SectSubStatus='No Records'  --- this never happen, this is just in case.		

				END

					UPDATE [dbo].[Empl] SET
										[Pub_Notes]=@ExistingPublicNotes,
										[SectStat]=@SectStatus,
