CREATE Procedure dbo.WS_EmailAttachment_Processor_GetAPNO ( @PartnerTrackingId varchar(20) = null, @DateEmailReceived DateTime,@Last varchar(100),@First varchar(100),@EmailFrom varchar(100),@apno int = null)
AS
SET NOCOUNT ON
Declare @TimeLag int
Declare @CCList nvarchar(1000)

Set @TimeLag = -24 --60 hour time lag to adjust for order placement and the recruiter sharing candidate profile.
set @CCList = 'DionnaAllen@Precheck.Com;JessicaViera@Precheck.com;CynthiaDoherty@Precheck.com;EmilyTorticil@Precheck.com'


Declare 
--@APNO Int,
@InvestigatorEmail nvarchar(100),@CAM_Email nvarchar(100)
select Top 1 @APNO = IsNull(@Apno,IOR.APNO),@InvestigatorEmail = isnull(U.EmailAddress,'') ,@CAM_Email = isnull(U2.EmailAddress,'') 
...
--IF (isnull(@InvestigatorEmail,'')<>'')
	BEGIN
		...
		BEGIN TRY
			...
			Set @RecipientList = @RecipientList    + '; SantoshChapyala@precheck.com; MistySmallwood@precheck.com;douglasdegenaro@precheck.com' 
		...
		END TRY
		BEGIN CATCH
			EXEC msdb.dbo.sp_send_dbmail @recipients='santoshchapyala@precheck.com;douglasdegenaro@precheck.com',
					@subject=N'Email Attachment notification',
