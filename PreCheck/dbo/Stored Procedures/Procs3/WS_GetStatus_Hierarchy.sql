...
--[dbo].[WS_GetStatus] 2167,'2XR_49_2167_524cc5f937667750',null,null,0,1,2324818




CREATE PROCEDURE [dbo].[WS_GetStatus_Hierarchy] 
	@CLNO int,
...
BEGIN
 ...
 Begin
  if (select count(1) from dbo.ReleaseForm where CLNO = @CLNO and ClientAPPNO = @ClientAppNo) > 0
		Select cast(1 as bit) 'ReleaseCaptured'
	else
		select cast(0 as bit) as 'ReleaseCaptured'
 End
 ...
 Else
	...
	IF (@ClientAppNo IS NOT NULL or @ApNo IS NOT NULL)
		BEGIN
			...
			IF @IncludeSSNDOB = 1
				SELECT @SSN = ISNULL(SSN,i94), @DOB = DOB 
...
--			
			...
			Declare @FacilityCLNO int,@AppCLNO INT
