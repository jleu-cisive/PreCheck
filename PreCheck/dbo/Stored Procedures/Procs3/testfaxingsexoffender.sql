-- Alter Procedure testfaxingsexoffender

CREATE PROCEDURE [dbo].[testfaxingsexoffender] @appno int,@county varchar(25),@cntyno int, @crimid int, @state varchar(2) AS
   ...
   begin
     declare @v1delivery varchar(25)
         set @v1delivery = (select r_delivery from iris_researchers where r_id = @vendor1)
            ...
            begin 
                 insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,readytosend,parentCrimID, crim_specialinstr)
                values(@appno,@county,@cntyno,@vendor1,@v1delivery,'Yes','Yes','0',@crimid, @state)
            end
              else
           begin
                insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,parentCrimID, crim_specialinstr )
                values(@appno,@county,@cntyno,@vendor1,@v1delivery,'Yes','Yes',@crimid, @state)
           end
    end
         ...
         declare @v2delivery varchar(25)
          set @v2delivery = (select r_delivery from iris_researchers where r_id = @vendor2)
             if (@v2delivery = 'Mail') or (@v2delivery = 'OnlineDB') or (@v2delivery = 'Call_In') or (@v2delivery = 'InHouse')
                begin
                  insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,readytosend,parentCrimID, crim_specialinstr)
                    values(@appno,@county,@cntyno,@vendor2,@v2delivery,'Yes','Yes','0',@crimid, @state)
                 end
             else
                begin
                     insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,parentCrimID, crim_specialinstr)
                values(@appno,@county,@cntyno,@vendor2,@v2delivery,'Yes','Yes',@crimid, @state)
              end
       end
...
if @vendor3 > 0
  begin
     ...
     begin         
          insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,readytosend,parentCrimID, crim_specialinstr)
                values(@appno,@county,@cntyno,@vendor3,@v3delivery,'Yes','Yes','0',@crimid, @state)
    end
             else
            begin
      insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,parentCrimID, crim_specialinstr)
                values(@appno,@county,@cntyno,@vendor3,@v3delivery,'Yes','Yes',@crimid, @state)
            end
           end
   ...
   begin
    ...
    set @v4delivery = (select r_delivery from iris_researchers where r_id = @vendor3)
           if (@v4delivery = 'Mail') or (@v4delivery = 'OnlineDB') or (@v4delivery = 'Call_In') or (@v4delivery = 'InHouse')
            begin
             insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,readytosend,parentCrimID, crim_specialinstr)
                values(@appno,@county,@cntyno,@vendor4,@v4delivery,'Yes','Yes','0',@crimid, @state)
             end
           else
         begin
            insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,parentCrimID, crim_specialinstr)
                values(@appno,@county,@cntyno,@vendor4,@v4delivery,'Yes','Yes',@crimid, @state)
        end
end
if @vendor5 > 0
  begin
...
set @v5delivery = (select r_delivery from iris_researchers where r_id = @vendor5)
     ...
     begin
        insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,readytosend,parentCrimID, crim_specialinstr)
                values(@appno,@county,@cntyno,@vendor5,@v5delivery,'Yes','Yes','0',@crimid, @state)
     end
     else
      begin
 insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,parentCrimID, crim_specialinstr)
                values(@appno,@county,@cntyno,@vendor5,@v5delivery,'Yes','Yes',@crimid, @state)
      end
    end
if @vendor6 > 0
  begin
declare @v6delivery varchar(25)
   ...
   begin 
  insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,readytosend,parentCrimID, crim_specialinstr)
                values(@appno,@county,@cntyno,@vendor6,@v6delivery,'Yes','Yes','0',@crimid, @state)  
   end
   else
   begin
   insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,parentCrimID, crim_specialinstr)
                values(@appno,@county,@cntyno,@vendor6,@v6delivery,'Yes','Yes',@crimid, @state)  
  end  
end--------------------------- THE END-----------------------------------
---------------- CHECK FOR DEFAULT --------------------
         ...
         If (@altvendoractive = '1' ) and (getdate() between @altvendorbeg and @altvendorend)
                 begin
                    set @c_vendorid = @altvendor
                         insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,readytosend,parentCrimID, crim_specialinstr)
                    values(@appno,@county,@c_county,@c_vendorid,@c_delivery,'Yes','yes',@checkreadytosend,@crimid, @state)
                 end
             else
                 Begin
                         insert into crim(apno,county,cnty_no,vendorid,deliverymethod,b_rule,iris_rec,readytosend,parentCrimID, crim_specialinstr)
                   values(@appno,@county,@cntyno,@c_vendorid,@c_delivery,'No','yes',@checkreadytosend,@crimid, @state)
                end
        END
