...
CREATE PROCEDURE [dbo].[Verification_Team_Production_Details] 
...
SELECT    Newvalue,(case when len(UserID) <=8 then UserID else  SUBSTRING ( UserID ,1 , len(UserID) -5) end) as UserID,id into #EmpltempChangeLog
FROM         dbo.ChangeLog
...
SELECT distinct (case when len(UserID) <=8 then UserID else  SUBSTRING ( UserID ,1 , len(UserID) -5) end) as UserID,id into #EmpltempChangeLog2
...
SELECT   (case when len(UserID) <=8 then UserID else  SUBSTRING ( UserID ,1 , len(UserID) -5) end) as UserID,id into #EmpltempChangeLog3
FROM         dbo.ChangeLog where
...
Select distinct UserID into #tempUsers from Users Where Empl = 1 OR Educat =1 OR PersRef = 1 and Disabled = 0 and CAM = 0 
order by UserID
----------------------------------------------------------------------
...
SELECT    Newvalue,(case when len(UserID) <=8 then UserID else  SUBSTRING ( UserID ,1 , len(UserID) -7) end) as UserID, id into #EdutempChangeLog
FROM         dbo.ChangeLog
...
SELECT    Newvalue,(case when len(UserID) <=8 then UserID else  SUBSTRING ( UserID ,1 , len(UserID) - 7) end) as UserID,id into #EdutempChangeLog2
FROM         dbo.ChangeLog where
...
SELECT   (case when len(UserID) <=8 then UserID else  SUBSTRING ( UserID ,1 , len(UserID) -7) end) as UserID,id into #EdutempChangeLog3
FROM         dbo.ChangeLog where
...
Insert into #TempVerificationProdDetails (Investigator, EmplEfforts, EduEfforts,PresRefEfforts,
		...
		Alert,UnVerified,ClosedbyUser, ClosedInUserModule, PendingAssigned,PendingOverall)
...
(Select count(1) From #TempStatus F2 (NoLock)  where Newvalue = 'C'  and isnull(F2.UserID,'') = isnull(T.UserID,'')) [ALERT],
(Select count(1) From #TempStatus F3 (NoLock)  where Newvalue = 'U'  and isnull(F3.UserID,'') = isnull(T.UserID,'')) [UNVERIFIED],
