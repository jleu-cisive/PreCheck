...
CREATE PROCEDURE [dbo].[ReportApplClientAverageTurnaroundReport_Module_AllClients]
(		
		 @from_date datetime='',
		 @to_date datetime='',
		 @section varchar(50)=''
...
WHERE empl.last_worked >= @from_date AND empl.last_worked < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( empl.CreatedDate, empl.last_worked ) = 0) as 'Count',
...
JOIN appl with (NOLOCK) on appl.apno = empl.apno
WHERE empl.last_worked >= @from_date AND empl.last_worked < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( empl.CreatedDate, empl.last_worked ) = 1) as 'Count',
CAST( 100. * (SELECT count(*) FROM empl with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = empl.apno
	WHERE empl.last_worked >= @from_date AND empl.last_worked < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = empl.apno
WHERE empl.last_worked >= @from_date AND empl.last_worked < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( empl.CreatedDate, empl.last_worked ) = 2) as 'Count',
CAST( 100. * (SELECT count(*) FROM empl with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = empl.apno
	WHERE empl.last_worked >= @from_date AND empl.last_worked < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = empl.apno
WHERE empl.last_worked >= @from_date AND empl.last_worked < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( empl.CreatedDate, empl.last_worked ) = 3) as 'Count',
CAST( 100. * (SELECT count(*) FROM empl with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = empl.apno
	WHERE empl.last_worked >= @from_date AND empl.last_worked < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = empl.apno
WHERE empl.last_worked >= @from_date AND empl.last_worked < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( empl.CreatedDate, empl.last_worked ) = 4) as 'Count',
...
JOIN appl with (NOLOCK) on appl.apno = empl.apno
WHERE empl.last_worked >= @from_date AND empl.last_worked < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( empl.CreatedDate, empl.last_worked ) = 5) as 'Count',
...
JOIN appl with (NOLOCK) on appl.apno = empl.apno
WHERE empl.last_worked >= @from_date AND empl.last_worked < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( empl.CreatedDate, empl.last_worked ) >= 6) as 'Count',
CAST( 100. * (SELECT count(*) FROM empl with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = empl.apno
	WHERE empl.last_worked >= @from_date AND empl.last_worked < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
WHERE educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date  and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND dbo.elapsedbusinessdays_2( educat.CreatedDate,educat.Web_Updated) = 0) as 'Count'
, CAST( 100. * (SELECT count(*) FROM educat with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = educat.apno
	WHERE 
educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date  and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = educat.apno
WHERE educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND dbo.elapsedbusinessdays_2( educat.CreatedDate,educat.Web_Updated) = 1) as 'Count'
, CAST( 100. * (SELECT count(*) FROM educat with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = educat.apno
	WHERE 
educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date  and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = educat.apno
WHERE educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND dbo.elapsedbusinessdays_2( educat.CreatedDate,educat.Web_Updated)  = 2) as 'Count'
, CAST( 100. * (SELECT count(*) FROM educat with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = educat.apno
	WHERE educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date  and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = educat.apno
WHERE educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND dbo.elapsedbusinessdays_2( educat.CreatedDate,educat.Web_Updated) = 3) as 'Count'
, CAST( 100. * (SELECT count(*) FROM educat with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = educat.apno
	WHERE 
educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date  and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = educat.apno
WHERE educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
 AND dbo.elapsedbusinessdays_2( educat.CreatedDate,educat.Web_Updated) = 4) as 'Count'
, CAST( 100. * (SELECT count(*) FROM educat with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = educat.apno
	WHERE 
educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date  and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
CAST( 100. * (SELECT count(*) FROM educat with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = educat.apno
	WHERE 
educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F') AND
...
JOIN appl with (NOLOCK) on appl.apno = educat.apno
WHERE educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
 AND dbo.elapsedbusinessdays_2( educat.CreatedDate,educat.Web_Updated) = 5) as 'Count'
, CAST( 100. * (SELECT count(*) FROM educat with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = educat.apno
	WHERE 
educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date  and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = educat.apno
WHERE educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND dbo.elapsedbusinessdays_2( educat.CreatedDate,educat.Web_Updated) >=6) as 'Count'
, CAST( 100. * (SELECT count(*) FROM educat with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = educat.apno
	WHERE 
educat.Web_Updated >= @from_date AND educat.Web_Updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
WHERE Crim.last_updated >= @from_date AND Crim.last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( Crim.Crimenteredtime, Crim.last_updated ) = 0) as 'Count',
...
JOIN appl with (NOLOCK) on appl.apno = Crim.apno
WHERE Crim.last_updated >= @from_date AND Crim.last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( Crim.Crimenteredtime, Crim.last_updated ) = 1) as 'Count',
CAST( 100. * (SELECT count(*) FROM Crim with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = Crim.apno
	WHERE Crim.Last_updated >= @from_date AND Crim.Last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = Crim.apno
WHERE Crim.last_updated >= @from_date AND Crim.last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( Crim.Crimenteredtime, Crim.last_updated ) = 2) as 'Count',
CAST( 100. * (SELECT count(*) FROM Crim with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = Crim.apno
	WHERE Crim.Last_updated >= @from_date AND Crim.Last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = Crim.apno
WHERE Crim.last_updated >= @from_date AND Crim.last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( Crim.Crimenteredtime, Crim.last_updated ) = 3) as 'Count',
CAST( 100. * (SELECT count(*) FROM Crim with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = Crim.apno
	WHERE Crim.Last_updated >= @from_date AND Crim.Last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = Crim.apno
WHERE Crim.last_updated >= @from_date AND Crim.last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( Crim.Crimenteredtime, Crim.last_updated ) = 4) as 'Count',
CAST( 100. * (SELECT count(*) FROM Crim with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = Crim.apno
	WHERE Crim.Last_updated >= @from_date AND Crim.Last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = Crim.apno
WHERE Crim.last_updated >= @from_date AND Crim.last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( Crim.Crimenteredtime, Crim.last_updated ) = 5) as 'Count',
CAST( 100. * (SELECT count(*) FROM Crim with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = Crim.apno
	WHERE Crim.Last_updated >= @from_date AND Crim.Last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = Crim.apno
WHERE Crim.Last_updated >= @from_date AND Crim.Last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( Crim.Crimenteredtime, Crim.Last_updated ) >= 6) as 'Count',
CAST( 100. * (SELECT count(*) FROM Crim with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = crim.apno
	WHERE Crim.Last_updated >= @from_date AND Crim.Last_updated < @to_date and ishidden = 0 AND apstatus in ('W','F')
...
WHERE ProfLic.Last_updated >= @from_date AND ProfLic.Last_updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( ProfLic.createdDate, ProfLic.Last_updated ) = 0) as 'Count',
...
JOIN appl with (NOLOCK) on appl.apno = profLic.apno
WHERE ProfLic.Last_updated >= @from_date AND ProfLic.Last_updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( ProfLic.createdDate, ProfLic.Last_updated ) = 1) as 'Count',
CAST( 100. * (SELECT count(*) FROM ProfLic with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = Proflic.apno
	WHERE ProfLic.Last_updated >= @from_date AND ProfLic.Last_updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = profLic.apno
WHERE ProfLic.Last_updated >= @from_date AND ProfLic.Last_updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( ProfLic.createdDate, ProfLic.Last_updated ) = 2) as 'Count',
CAST( 100. * (SELECT count(*) FROM ProfLic with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = Proflic.apno
	WHERE ProfLic.Last_updated >= @from_date AND ProfLic.Last_updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = profLic.apno
WHERE ProfLic.Last_updated >= @from_date AND ProfLic.Last_updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( ProfLic.createdDate, ProfLic.Last_updated ) = 3) as 'Count',
CAST( 100. * (SELECT count(*) FROM ProfLic with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = Proflic.apno
	WHERE ProfLic.Last_updated >= @from_date AND ProfLic.Last_updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = profLic.apno
WHERE ProfLic.Last_updated >= @from_date AND ProfLic.Last_updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( ProfLic.createdDate, ProfLic.Last_updated ) = 4) as 'Count',
CAST( 100. * (SELECT count(*) FROM ProfLic with (NOLOCK)
...
JOIN appl with (NOLOCK) on appl.apno = profLic.apno
WHERE ProfLic.Last_updated >= @from_date AND ProfLic.Last_updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( ProfLic.createdDate, ProfLic.Last_updated ) = 5) as 'Count',
CAST( 100. * (SELECT count(*) FROM ProfLic with (NOLOCK)
	JOIN appl on appl.apno = Proflic.apno
	WHERE ProfLic.Last_updated >= @from_date AND ProfLic.Last_updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
JOIN appl with (NOLOCK) on appl.apno = profLic.apno
WHERE ProfLic.Last_updated >= @from_date AND ProfLic.Last_updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND  dbo.elapsedbusinessdays_2( ProfLic.createdDate, ProfLic.Last_updated ) >=6) as 'Count',
CAST( 100. * (SELECT count(*) FROM ProfLic with (NOLOCK)
	JOIN appl with (NOLOCK) on appl.apno = Proflic.apno
	WHERE ProfLic.Last_updated >= @from_date AND ProfLic.Last_updated < @to_date and ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
...
(CASE  
	...
	WHEN isnull(PersRef.Web_Updated,'1/1/1900') >= isnull(PersRef.last_worked,'1/1/1900') AND isnull(PersRef.Web_Updated,'1/1/1900') >= isnull(PersRef.last_updated,'1/1/1900') THEN PersRef.Web_Updated 
END)  < @to_date
AND ishidden = 0 and isonreport = 1
AND apstatus in ('W','F')
...
(CASE  
	...
	WHEN isnull(PersRef.last_worked,'1/1/1900') >= isnull(PersRef.last_updated,'1/1/1900') AND isnull(PersRef.last_worked,'1/1/1900') >= isnull(PersRef.Web_Updated,'1/1/1900') THEN PersRef.last_worked 
	WHEN isnull(PersRef.Web_Updated,'1/1/1900') >= isnull(PersRef.last_worked,'1/1/1900') AND isnull(PersRef.Web_Updated,'1/1/1900') >= isnull(PersRef.last_updated,'1/1/1900') THEN PersRef.Web_Updated 
END)  < @to_date AND ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
  )AS NUMERIC( 7, 2 ) ) as 'Cumulative %'
FROM PersRef with (NOLOCK) JOIN appl with (NOLOCK) on appl.apno = PersRef.apno
WHERE  
(CASE  
...
(CASE  
	...
	WHEN isnull(PersRef.Web_Updated,'1/1/1900') >= isnull(PersRef.last_worked,'1/1/1900') AND isnull(PersRef.Web_Updated,'1/1/1900') >= isnull(PersRef.last_updated,'1/1/1900') THEN PersRef.Web_Updated 
END)  < @to_date
AND ishidden = 0 and isonreport = 1 AND apstatus in ('W','F')
AND (CASE  
	WHEN isnull(PersRef.last_updated,'1/1/1900') >= isnull(PersRef.last_worked,'1/1/1900') AND isnull(PersRef.last_updated,'1/1/1900') >= isnull(PersRef.Web_Updated,'1/1/1900') THEN dbo.elapsedbusinessdays_2( PersRef.CreatedDate,PersRef.last_updated)  
	WHEN isnull(PersRef.last_worked,'1/1/1900') >= isnull(PersRef.last_updated,'1/1/1900') AND isnull(PersRef.last_worked,'1/1/1900') >= isnull(PersRef.Web_Updated,'1/1/1900') THEN dbo.elapsedbusinessdays_2( PersRef.CreatedDate,PersRef.last_worked) 
	WHEN isnull(PersRef.Web_Updated,'1/1/1900') >= isnull(PersRef.last_worked,'1/1/1900') AND isnull(PersRef.Web_Updated,'1/1/1900') >= isnull(PersRef.last_updated,'1/1/1900') THEN dbo.elapsedbusinessdays_2( PersRef.CreatedDate,PersRef.Web_Updated)
