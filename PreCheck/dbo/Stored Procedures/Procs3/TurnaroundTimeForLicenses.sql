CREATE PROCEDURE [dbo].[TurnaroundTimeForLicenses]  --'04/01/2015','04/23/2015'
(
  @StartDate datetime,
  @EndDate datetime
)
...
select case when dbo.elapsedbusinessdays_2( convert(datetime,pl.CreatedDate), pl.Last_updated) < 6 then dbo.elapsedbusinessdays_2( convert(datetime,pl.CreatedDate), pl.Last_updated) 
  ...
  from ProfLic pl with (nolock)
  join Appl 
    on Appl.Apno = pl.Apno
...
,appl.apno, apdate as 'App Created Date',pl.Lic_Type as LicenseType,pl.Last_updated as LicenseCompletedDate
from appl with (nolock) inner join ProfLic pl on Appl.apno = pl.apno
where Apdate >= @StartDate and pl.Last_updated < @EndDate and pl.SectStat not in ('9')
group by appl.apno, apdate,pl.Lic_Type,pl.Last_updated
...
--from appl with (nolock) where Apdate >= @StartDate and Apdate < @EndDate and apstatus in ('W','F')
