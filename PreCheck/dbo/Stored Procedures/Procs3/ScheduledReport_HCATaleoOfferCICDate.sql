...
-- Modified by AmyLiu: 10/11/2022 Change to 60 days in the past per Laura
-- =============================================    
CREATE PROCEDURE [dbo].[ScheduledReport_HCATaleoOfferCICDate]     
...
BEGIN    
 ...
 IF(@StartDate IS NULL)    
  set @StartDate =DATEADD(d,-60,GETDATE())  ---30 days in the past    
 IF(@EndDate IS NULL)    
 ...
 SELECT      
  ...
  ApplicantId = ISNULL(A.ClientCandidateId,'0'),    
     replace(A.email,',',' ')  as [ApplicantEmail],    
  ...
  -- SSN = case when @IncldueSSN=1 then SocialNumber else (case when isnull(SocialNumber,'')<>'' then 'xxx-xx-'+right(SocialNumber,4) else '' end) end
  SSN = case when isnull(SocialNumber,'')<>'' then 'xxx-xx-'+right(SocialNumber,4) else '' end
