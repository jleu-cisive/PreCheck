...
--EXEC [TestEnvReportApplClientAverageTurnaroundReport] 0,'3/1/2017','3/31/2017','MD Anderson'
...
CREATE PROCEDURE [dbo].[TestEnvReportApplClientAverageTurnaroundReport] 
@CLNO int, 
@from_date datetime, 
@to_date datetime,
...
(select COUNT( A.APNO ) from [Hou-SQLTEST-01].PreCheck.dbo.appl A with (nolock) 
inner join [Hou-SQLTEST-01].PreCheck.dbo.Client C on A.CLNO = C.CLNO
inner join [Hou-SQLTEST-01].PreCheck.dbo.refAffiliate RA on RA.AffiliateID = C.AffiliateID
...
CAST( 100. * (select COUNT( A.APNO ) from [Hou-SQLTEST-01].PreCheck.dbo.appl A with (nolock) 
              inner join [Hou-SQLTEST-01].PreCheck.dbo.Client C on A.CLNO = C.CLNO
			  inner join [Hou-SQLTEST-01].PreCheck.dbo.refAffiliate RA on RA.AffiliateID = C.AffiliateID
...
from [Hou-SQLTEST-01].PreCheck.dbo.appl ap with (nolock) 
   inner join [Hou-SQLTEST-01].PreCheck.dbo.Client C on Ap.CLNO = C.CLNO
   inner join [Hou-SQLTEST-01].PreCheck.dbo.refAffiliate RA on RA.AffiliateID = C.AffiliateID
...
group by RA.Affiliate), 
(SELECT SUM(dbo.elapsedbusinessdays_2( Apdate, Origcompdate )) FROM [Hou-SQLTEST-01].PreCheck.dbo.appl A with (nolock) inner join [Hou-SQLTEST-01].PreCheck.dbo.Client C on A.CLNO = C.CLNO inner join [Hou-SQLTEST-01].PreCheck.dbo.refAffiliate RA on RA.AffiliateID = C.AffiliateID 
   where Apdate >= @from_date and Apdate < @to_date and (@CLNO=0 OR A.CLNO = @CLNO) and (@Affiliate='' or LTRIM(RTRIM(RA.Affiliate)) = @Affiliate) and apstatus in ('W','F') and ( dbo.elapsedbusinessdays_2( Apdate, Origcompdate )) = 1 -- + dbo.elapsedbusinessdays_2( Reopendate, Compdate ) ) = 1 
   group by RA.Affiliate),
CAST( 100. * (select COUNT( A.APNO ) from [Hou-SQLTEST-01].PreCheck.dbo.appl A with (nolock) 
   inner join [Hou-SQLTEST-01].PreCheck.dbo.Client C on A.CLNO = C.CLNO
...
inner join [Hou-SQLTEST-01].PreCheck.dbo.refAffiliate RA on RA.AffiliateID = C.AffiliateID 
  ...
  (select count( A.APNO ) from [Hou-SQLTEST-01].PreCheck.dbo.appl A with (nolock) 
   ...
   where Apdate >= @from_date and Apdate < @to_date and (@CLNO=0 OR A.CLNO = @CLNO) and (@Affiliate='' or LTRIM(RTRIM(RA.Affiliate)) = @Affiliate) and apstatus in ('W','F') --11
   group by RA.Affiliate) AS NUMERIC( 5, 2 ) ),
CAST( 100. * (select COUNT( APNO ) from [Hou-SQLTEST-01].PreCheck.dbo.appl A with (nolock) 
inner join [Hou-SQLTEST-01].PreCheck.dbo.Client C on A.CLNO = C.CLNO
inner join [Hou-SQLTEST-01].PreCheck.dbo.refAffiliate RA on RA.AffiliateID = C.AffiliateID 
   where Apdate >= @from_date and Apdate < @to_date and (@CLNO=0 OR A.CLNO = @CLNO) and (@Affiliate='' or LTRIM(RTRIM(RA.Affiliate)) = @Affiliate) and apstatus in ('W','F') and ( dbo.elapsedbusinessdays_2( Apdate, Origcompdate ))<=4 -- + dbo.elapsedbusinessdays_2( Reopendate, Compdate ) ) <= 4 
   group by RA.Affiliate) /
...
group by RA.Affiliate),
(SELECT SUM(dbo.elapsedbusinessdays_2( Apdate, Origcompdate )) FROM [Hou-SQLTEST-01].PreCheck.dbo.appl A with (nolock) inner join Client C on A.CLNO = C.CLNO inner join [Hou-SQLTEST-01].PreCheck.dbo.refAffiliate RA on RA.AffiliateID = C.AffiliateID 
...
(@CLNO=0 OR A.CLNO = @CLNO) and (@Affiliate='' or RA.Affiliate = @Affiliate) and apstatus in ('W','F') and ( dbo.elapsedbusinessdays_2( Apdate, Origcompdate )) > = 0 -- + dbo.elapsedbusinessdays_2( Reopendate, Compdate ) ) >= 0
   group by RA.Affiliate) /
  (select count( A.APNO ) from [Hou-SQLTEST-01].PreCheck.dbo.appl A with (nolock) 
  inner join Client C on A.CLNO = C.CLNO
  inner join [Hou-SQLTEST-01].PreCheck.dbo.refAffiliate RA on RA.AffiliateID = C.AffiliateID 
