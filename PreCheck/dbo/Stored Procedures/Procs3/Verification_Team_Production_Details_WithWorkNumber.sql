...
/*
	EXEC [Verification_Team_Production_Details_WithWorkNumber] '07/06/2020','07/06/2020'

-- =============================================
-- Author:		Originally created by No idea
-- Create date: 
-- Description:
-- Modified by Radhika Dereddy on 02/26/2020 per Pam/Michelle request
-- =============================================
*/

CREATE PROCEDURE [dbo].[Verification_Team_Production_Details_WithWorkNumber]
@StartDate DateTime,
@EndDate DateTime
...
Select count(EmplID),  H.Investigator as UserID From Empl H (NoLock) where H.sectstat in ('A','4','5','6','7','8','C','U') and 
Last_Worked>= @StartDate and Last_Worked< =@EndDate and H.Investigator is not null
group by H.Investigator	   
insert into #tempAllModulesClosed (ClosedInUserModule, UserID)
Select count(EducatID), I.Investigator as UserID From Educat I (NoLock)  where I.sectstat in ('A','4','5','6','7','8','C','U') and
...
Select count(PersRefID), J.Investigator as UserID From PersRef J (NoLock)  where J.sectstat in ('A','4','5','6','7','8','C','U') 
...
(
	...
	PersRefVerified int,
	PersRefSeeAttached int, 
	PersRefUnVerified int, 
	PersRefAlert int,
...
Insert into #TempVerificationProdDetails (Investigator, EmplEfforts, EduEfforts,PresRefEfforts,
		...
		EmplUpdatedWebStatus, EduUpdatedWebStatus,PersRefUpdatedWebStatus,PersRefVerified, PersRefSeeAttached, PersRefUnVerified, PersRefAlert, PersRefVerifiedSeeAttached,PersRefUnVerifiedSeeAttached,
		Verified,VerifiedSeeAttached,UnVerifiedSeeAttached,SeeAttached,AlertSeeAttached,EduInstClosed,Alert,UnVerified,
...
(Select count(1) From #PersRefTempStatus x (NoLock)  where Newvalue = '4'  and isnull(x.UserID,'') = isnull(T.UserID,'')) [Pers Ref - VERIFIED],
(Select count(1) From #PersRefTempStatus x (NoLock)  where Newvalue = '8'  and isnull(x.UserID,'') = isnull(T.UserID,'')) [Pers Ref - SEE ATTACHED],
(Select count(1) From #PersRefTempStatus x (NoLock)  where Newvalue = 'U'  and isnull(x.UserID,'') = isnull(T.UserID,'')) [Pers Ref - UNVERIFIED],
(Select count(1) From #PersRefTempStatus x (NoLock)  where Newvalue = 'C'  and isnull(x.UserID,'') = isnull(T.UserID,'')) [Pers Ref - ALERT],
...
(Select count(1) From #TempStatus F1 (NoLock)  where Newvalue = 'C'  and isnull(F1.UserID,'') = isnull(T.UserID,'')) [ALERT],
(Select count(1) From #TempStatus F1 (NoLock)  where Newvalue = 'U'  and isnull(F1.UserID,'') = isnull(T.UserID,'')) [UNVERIFIED],

(Select count(1) From #TempStatus G (NoLock)  where G.Newvalue in ('A','4','5','6','8','7','C','U') and isnull(G.UserID,'') = isnull(T.UserID,'')) [Closed by User],
...
sum(PersRefVerified) as [Pers Ref - VERIFIED], 
sum(PersRefSeeAttached) as [Pers Ref - SEE ATTACHED], 
sum(PersRefUnVerified) as [Pers Ref - UNVERIFIED], 
sum(PersRefAlert) as [Pers Ref - ALERT],
sum(PersRefVerifiedSeeAttached) as [Pers Ref - VERIFIED/SEE ATTACHED],
