...
/*
EXEC [ReportApplClientAverageTurnaroundReport_Test] 0,'07/1/2017','07/31/2017','MD Anderson'
EXEC [ReportApplClientAverageTurnaroundReport_Test] 0,'07/1/2017','07/31/2017',null
*/
...
CREATE PROCEDURE [dbo].[ReportApplClientAverageTurnaroundReport_Test] 
@CLNO int, 
@from_date datetime, 
@to_date datetime--,
...
(
	...
	where A.Apdate >= @from_date and A.Apdate < @to_date and (@CLNO=0 OR A.CLNO = @CLNO) --and (@Affiliate='' or LTRIM(RTRIM(RA.Affiliate)) = @Affiliate) 
	...
	CAST( 100. * (select COUNT( A.APNO ) from appl A with (nolock) 
				  inner join Client C on A.CLNO = C.CLNO
				  inner join refAffiliate RA on RA.AffiliateID = C.AffiliateID
	   where Apdate >= @from_date and Apdate < @to_date and (@CLNO=0 OR A.CLNO = @CLNO) --and (@Affiliate='' or LTRIM(RTRIM(RA.Affiliate)) = @Affiliate) 
	     and apstatus in ('W','F') and ( dbo.elapsedbusinessdays_2( Apdate, Origcompdate ))=0 -- + dbo.elapsedbusinessdays_2( Reopendate, Compdate ) ) = 0 
...
UNION
	...
	inner join refAffiliate RA on RA.AffiliateID = C.AffiliateID 
	   where Apdate >= @from_date and Apdate < @to_date and
			(@CLNO=0 OR A.CLNO = @CLNO) --and (@Affiliate='' or RA.Affiliate = @Affiliate) 
	    and apstatus in ('W','F') and ( dbo.elapsedbusinessdays_2( Apdate, Origcompdate )) > = 0 -- + dbo.elapsedbusinessdays_2( Reopendate, Compdate ) ) >= 0
	   group by RA.Affiliate) /
	  (select count( A.APNO ) from appl A with (nolock) 
	  inner join Client C on A.CLNO = C.CLNO
