...
-- Description:	Top6_CrimVendor_BillingDetails
-- EXEC Top6_CrimVendor_BillingDetails   ----SEX OFFENDER SEARCH, INNOVATIVE NATIONAL,WHOLESALE SCREENING
-- =============================================
CREATE PROCEDURE [dbo].[Top6_CrimVendor_BillingDetails] 
AS
BEGIN
	...
	AND CAST(cr.ordered as DATE) >= DATEADD(day, -30, getdate())
	AND (r.R_Name like '%SJV%' OR r.R_Name like '%Securitec%'  OR r.R_Name like '%Lexis%' OR r.R_Name like '%Omni%' OR r.R_Name like '%Wholesale%' OR r.R_Name like '%Innovative%') 
