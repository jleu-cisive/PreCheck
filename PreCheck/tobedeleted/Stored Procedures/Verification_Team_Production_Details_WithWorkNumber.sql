...
/*
	EXEC [Verification_Team_Production_Details_WithWorkNumber] '09/16/2019','09/16/2019'
*/

CREATE PROCEDURE [tobedeleted].[Verification_Team_Production_Details_WithWorkNumber] --'04/12/2017', '11/30/2015'
--DECLARE
@StartDate DateTime,-- = '09/16/2019', 
@EndDate DateTime--- = '09/16/2019'
...
if @StartDate is null 
	Select @StartDate = CAST(CURRENT_TIMESTAMP AS date)

if @EndDate is null 
	Select @EndDate = CAST(CURRENT_TIMESTAMP AS date)

	set @enddatelocal = @EndDate+1--dateadd(d,1,@EndDate)
...
WHERE (c.changedate between @StartDate and @EndDateLocal and i.refVerificationSource = 'WorkNumber' and i.DateTimStamp between @StartDate and @EndDateLocal)
...
having (max(changedate) between @StartDate and @EndDateLocal)
and (TableName = 'Empl.sectstat')
)c1
ON c.UserID = c1.UserId and c.id = c1.id and c.changedate = c1.changedate
JOIN dbo.Integration_Verification_SourceCode i (nolock) 
ON c1.id = i.sectionkeyid
WHERE (c1.changedate between @StartDate and @EndDateLocal and i.refVerificationSource = 'WorkNumber' and i.DateTimStamp between @StartDate and @EndDateLocal)
...
and c.ChangeDate between @StartDate and @EndDateLocal
order by c.UserID
...
Where ((InvestigatorAssigned>= @StartDate and InvestigatorAssigned< = @EndDateLocal)
OR (Last_Worked>= @StartDate and Last_Worked< = @EndDateLocal))
and emplid in (
SELECT sectionkeyid
FROM         dbo.Integration_Verification_SourceCode
WHERE    (DateTimStamp between @StartDate and @EndDateLocal and refVerificationSource = 'WorkNumber' and DateTimStamp between @StartDate and @EndDateLocal)
)
...
and  ChangeDate between @StartDate and @EndDateLocal
...
Where (Last_Worked>= @StartDate and Last_Worked< = @EndDateLocal)
order by Investigator asc
...
Last_Worked>= @StartDate and Last_Worked< =@EndDateLocal and H.Investigator is not null
...
Last_Worked>= @StartDate and Last_Worked< = @EndDateLocal and I.Investigator is not null
...
and Last_Worked>= @StartDate and Last_Worked< = @EndDateLocal and J.Investigator is not null 
...
(Select count(1) From dbo.#Empltmp1 X (NoLock) where  (InvestigatorAssigned>= @StartDate and InvestigatorAssigned< = @EndDateLocal) and isnull(X.Investigator,'') = isnull(T.UserID,'')) [Empl Verifications Assigned],
 0 [Edu Verifications Assigned],
 0 [PersRef Verifications Assigned],
--(Select count(1) From dbo.#Edutmp1 Y (NoLock) where  (InvestigatorAssigned>= @StartDate and InvestigatorAssigned< = dateadd(s,-1,dateadd(d,1,@EndDateLocal))) and isnull(Y.Investigator,'') = isnull(T.UserID,'')) [Edu_Verifications_Assigned],
--(Select count(1) From dbo.#PersReftmp1 Z (NoLock) where  (InvestigatorAssigned>= @StartDate and InvestigatorAssigned< = dateadd(s,-1,dateadd(d,1,@EndDateLocal))) and isnull(Z.Investigator,'') = isnull(T.UserID,'')) [Empl_Verifications_Assigned],
