...
CREATE VIEW [CTCFG].[vwAppl_ApStatus_New] AS 
						...
						UNION 
						SELECT DISTINCT N.APNO, NULL AS Old_ApStatus, N.ApStatus AS New_ApStatus, T.tran_end_time AS CommitDateTime, N.LastModifiedBy AS LastModifiedBy FROM Precheck.[CDC].[dbo_Appl_CT]  N
						LEFT OUTER  JOIN [Precheck].cdc.lsn_time_mapping T ON N.__$start_lsn = Start_lsn
						WHERE N.__$operation= 2;
